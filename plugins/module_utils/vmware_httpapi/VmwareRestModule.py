...
FILTER = dict(
    clusters=dict(
        name="clusters", id="id", type="str", format=r"domain\-[0-9a-fA-F]+"
    ),
    ...
    datastore_types=dict(
        ...
        id="type",
        type="list",
        choices=["", "CIFS", "NFS", "NFS41", "VFFS", "VMFS", "VSAN", "VVOL"],
    ),
    datastores=dict(
        name="datastores",
        id="id",
    ...
    folders=dict(
        name="folders", id="id", type="str", format=r"group\-[hnv][0-9a-fA-F]+"
    ),
    hosts=dict(
        name="hosts", id="id", type="str", format=r"host\-[0-9a-fA-F]+"
    ),
    names=dict(name="names", id="name", type="str", format=r".+"),
    network_types=dict(
        name="types",
        id="type",
        type="list",
    ...
    power_states=dict(
        ...
        id="power state",
        type="list",
        choices=["", "POWERED_OFF", "POWERED_ON", "SUSPENDED"],
    ),
    resource_pools=dict(
        name="resource_pools",
        id="id",
    ...
    vms=dict(name="vms", id="id", type="str", format=r"vm\-[0-9a-fA-F]+"),
)
...
INVENTORY = dict(
    category=dict(api="cis", url="/tagging/category", filters=[]),
    cluster=dict(
        api="vcenter",
        url="/cluster",
        filters=["clusters", "datacenters", "folders", "names"],
    ),
    content_library=dict(api="content", url="/library", filters=[]),
    content_type=dict(api="content", url="/type", filters=[]),
    datacenter=dict(
        api="vcenter",
        url="/datacenter",
        filters=["datacenters", "folders", "names"],
    ),
    datastore=dict(
        api="vcenter",
        url="/datastore",
    ...
    local_library=dict(api="content", url="/local-library", filters=[]),
    network=dict(
        api="vcenter",
        url="/network",
        filters=[
    ...
    storage_policy=dict(
        ...
        filters=["policies", "status", "vms"],
    ),
    subscribed_library=dict(
        api="content", url="/subscribed-library", filters=[]
    ),
    tag=dict(api="cis", url="/tagging/tag", filters=[]),
    session=dict(api="cis", url="/session", filters=[]),
    vm=dict(
        api="vcenter",
        url="/vm",
        filters=[
...
class VmwareRestModule(AnsibleModule):
    def __init__(
        ...
        self.request = dict(url=None, filter=None, data={}, method=None)

        # Last response from httpapi
        self.response = dict(status=None, data={})
    ...
    def handle_default_success(self):
        ...
        if self.log_level == "debug":
            self.result[self.key].update(debug=self._api_debug())
    ...
    def handle_default_401(self):
        ...
        self.fail(
            msg="Unable to authenticate. Provided credentials are not valid."
        )
    ...
    def handle_default_object(self):
        ...
        try:
            msg = self.response["data"]["value"]["messages"][0][
                "default_message"
            ]
    ...
    def handle_object_key_error(self):
        ...
        msg = (
            "Please specify correct object type to get information, "
    ...
    def get(self, url="/rest", key="result"):
        ...
        self.key = key
        self.response["status"], self.response[
            "data"
        ] = self._connection.send_request(url, {}, method="GET")
        self._use_handler()
    ...
    def put(self, url="/rest", data=None, key="result"):
        ...
        key.
        """
        self.request.update(url=url, data=data, method="PUT")
        self.key = key
        self.response["status"], self.response[
            "data"
        ] = self._connection.send_request(url, data, method="PUT")
        self._use_handler()
    ...
    def delete(self, url="/rest", data="result", key="result"):
        ...
        key.
        """
        self.request.update(url=url, data=data, method="DELETE")
        self.key = key
        self.response["status"], self.response[
            "data"
        ] = self._connection.send_request(url, data, method="DELETE")
        self._use_handler()
    ...
    def get_id(self, object_type, name):
        ...
        try:
            url = (
                API[INVENTORY[object_type]["api"]]["base"]
            ...
            self.fail(
                msg="object_type must be one of [%s]."
        ...
        if not self.allow_multiples and num_items > 1:
            msg = (
                "Found %d objects of type %s with name %s. "
                ...
                for filter in self.filters:
                    for key in list(filter.keys()):
                        ...
                        if filter_key not in INVENTORY[object_type]["filters"]:
                            msg = (
                                "%s is not a valid %s filter, choices are [%s]."
                                % (
                                    key,
                                    object_type,
                                    ", ".join(
                                        INVENTORY[object_type]["filters"]
                                    ),
                                )
                            )
                            self.fail(msg=msg)
                        ...
                        if (
                            FILTER[filter_key]["type"] == "str"
                            and not re.match(
                                FILTER[filter_key]["format"], filter[key]
                            )
                        ) or (
                            FILTER[filter_key]["type"] == "list"
                            and filter[key]
                            not in FILTER[filter_key]["choices"]
                        ):
    ...
    def reset(self):
        ...
        self.request.update(url=None, data={}, method=None)
        self.response.update(status=None, data={})
