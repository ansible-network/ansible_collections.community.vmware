...
class VmwareServiceManager(PyVmomi):
    ...
    def service_ctrl(self):
        ...
        for host in self.hosts:
            actual_service_state, actual_service_policy = self.check_service_state(
                host=host, service_name=self.service_name
            )
                ...
                try:
                    if self.desired_state in ["start", "present"]:
                        if not actual_service_state:
                            if not self.module.check_mode:
                                host_service_system.StartService(
                        ...
                        if actual_service_state:
                            if not self.module.check_mode:
                                host_service_system.StopService(
                                    id=self.service_name
                                )
                            changed_state = True
                    elif self.desired_state == "restart":
                        if not self.module.check_mode:
                            host_service_system.RestartService(
                                id=self.service_name
                            )
                        changed_state = True
                    ...
                    if self.desired_policy:
                        if actual_service_policy != self.desired_policy:
                            if not self.module.check_mode:
                                host_service_system.UpdateServicePolicy(
                                    id=self.service_name,
                    ...
                    self.results[host.name].update(
                        changed=False, error=to_native(e.msg)
                    )

        if any(host_service_state):
            changed = True
        self.module.exit_json(
            changed=changed, host_service_status=self.results
        )
    ...
    def check_service_state(self, host, service_name):
        ...
        msg = "Failed to find '%s' service on host system '%s'" % (
            service_name,
            host.name,
        )
        cluster_name = self.params.get("cluster_name", None)
        ...
        msg += (
            ", please check if you have specified a valid ESXi service name."
        )
