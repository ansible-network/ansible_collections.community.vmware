    ...
    def send_key_to_vm(self, vm_obj):
        ...
        if self.params["keys_send"]:
            for specified_key in self.params["keys_send"]:
                ...
                for keys in self.keys_hid_code:
                    if (
                        isinstance(keys[0], tuple) and specified_key in keys[0]
                    ) or (
                        not isinstance(keys[0], tuple)
                        and specified_key == keys[0]
                    ):
                        hid_code, modifiers = self.get_hid_from_key(
                            specified_key
                        )
                ...
                if not key_found:
                    self.module.fail_json(
                        msg="keys_send parameter: '%s' in %s not supported."
        ...
        if self.params["string_send"]:
            for char in self.params["string_send"]:
                ...
                for keys in self.keys_hid_code:
                    if (
                        isinstance(keys[0], tuple) and char in keys[0]
                    ) or char == " ":
                ...
                if not key_found:
                    self.module.fail_json(
                        msg="string_send parameter: '%s' contains char: '%s' not supported."
                        % (self.params["string_send"], char)
                    )

        if self.usb_scan_code_spec.keyEvents:
            try:
                num_keys_returned = vm_obj.PutUsbScanCodes(
                    self.usb_scan_code_spec
                )
                self.change_detected = True
            except vmodl.RuntimeFault as e:
                self.module.fail_json(
                    msg="Failed to send key %s to virtual machine due to %s"
                    % (key_event, to_native(e.msg))
                )
