...
class VmwareHostDNS(PyVmomi):
        ...
        for host in self.hosts:
            ...
            for instance in host_netstack_config:
                if instance.key == "defaultTcpipStack":
                    ...
                    netstack_spec.netStackInstance = (
                    ...
                    if self.network_type == "static":
                        ...
                        if self.search_domains:
                            if (
                                instance.dnsConfig.searchDomain
                                != self.search_domains
                            ):
                                host_result[
                        ...
                        and not instance.dnsConfig.dhcp
                    ):
            ...
            if changed:
                ...
                if not self.module.check_mode:
                    try:
                        ...
                        self.module.fail_json(
                            msg="Network entity specified in the configuration already exist on host '%s'"
                            % host.name
                        )
                    except vim.fault.NotFound:
                        self.module.fail_json(
                            msg="Network entity specified in the configuration doesn't exist on host '%s'"
                            % host.name
                        )
                    except vim.fault.ResourceInUse:
                        self.module.fail_json(
                            msg="Resource is in use on host '%s'" % host.name
                        )
    ...
    def build_changed_message(self, dns_servers_configured, dns_servers_new):
        ...
        remove = self.get_not_in_list_one(
            dns_servers_configured, dns_servers_new
        )
        ...
        if add and remove:
            ...
            if dns_servers_new != diff_servers:
                ...
                if dns_servers_new != dns_servers_configured:
                    message = "DNS server %s %sreplaced with %s" % (
                        self.array_to_string(remove),
                        check_mode,
                        self.array_to_string(add),
                    )
                else:
                    message = "DNS server %s %sremoved and %s %sadded" % (
                        self.array_to_string(remove),
                        check_mode,
                        self.array_to_string(add),
                        check_mode,
