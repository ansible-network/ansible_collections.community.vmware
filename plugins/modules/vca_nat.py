...
def validate_nat_rules(nat_rules):
    for rule in nat_rules:
        if not isinstance(rule, dict):
            raise VcaError(
                "nat rules must be a list of dictionaries, " "Please check",
        ...
        for k in rule.keys():
            if k not in VALID_RULE_KEYS:
                raise VcaError(
                    "%s is not a valid key in nat rules, please "
                    "check above.." % k,
                    valid_keys=VALID_RULE_KEYS,
                )
        ...
        rule["translated_port"] = str(
            rule.get("translated_port", "any")
        ).lower()
    ...
    for rule in nat_rules:
        ...
        result.append(
            dict(
                ...
                translated_port=(
                    gw_rule.get_TranslatedPort().lower() or "any"
                ),
