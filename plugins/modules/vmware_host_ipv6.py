        ...
        for host in self.hosts:
            ...
            if desired_state == "enabled":
                ...
                if host_network_info.atBootIpV6Enabled:
                    if host_network_info.ipV6Enabled:
                        results["result"][host.name]["msg"] = (
                            "IPv6 is already enabled and active for host '%s'"
                            % host.name
                        )
                    if not host_network_info.ipV6Enabled:
                        results["result"][host.name]["msg"] = (
                            "IPv6 is already enabled for host '%s', but a reboot"
                ...
                # Enable IPv6
                    ...
                    if not self.module.check_mode:
                        try:
                            ...
                            results["result"][host.name]["msg"] = (
                                "IPv6 enabled for host '%s'" % host.name
                            )
                        except (vim.fault.AlreadyExists, vim.fault.NotFound):
                            self.module.fail_json(
                                msg="Network entity specified in the configuration for host '%s'"
                                " already exists" % host.name
                            )
                        except vmodl.fault.InvalidArgument as invalid_argument:
                            self.module.fail_json(
                                msg="Invalid parameter specified for host '%s' : %s"
                                % (host.name, to_native(invalid_argument.msg))
                            )
                        except vim.fault.HostConfigFault as config_fault:
                            self.module.fail_json(
                                msg="Failed to enable IPv6 for host '%s' due to : %s"
                        ...
                        results["result"][host.name]["changed"] = True
                        results["result"][host.name]["msg"] = (
                            "IPv6 will be enabled for host '%s'" % host.name
                        )
                ...
                if not host_network_info.atBootIpV6Enabled:
                    if not host_network_info.ipV6Enabled:
                        results["result"][host.name]["msg"] = (
                            "IPv6 is already disabled for host '%s'"
                            % host.name
                        )
                    if host_network_info.ipV6Enabled:
                        changed = True
                ...
                # Disable IPv6
                    ...
                    if not self.module.check_mode:
                        try:
                            ...
                            results["result"][host.name]["changed"] = True
                            results["result"][host.name]["msg"] = (
                                "IPv6 disabled for host '%s'" % host.name
                            )
                        except (vim.fault.AlreadyExists, vim.fault.NotFound):
                            self.module.fail_json(
                                msg="Network entity specified in the configuration for host '%s'"
                            ...
                            self.module.fail_json(
                                msg="Failed to disable IPv6 for host '%s' due to : %s"
                                % (host.name, to_native(config_fault.msg))
                            )
                        except vmodl.fault.NotSupported as not_supported:
                            self.module.fail_json(
                        ...
                        results["result"][host.name]["changed"] = True
                        results["result"][host.name]["msg"] = (
                            "IPv6 will be disabled for host '%s'" % host.name
                        )
