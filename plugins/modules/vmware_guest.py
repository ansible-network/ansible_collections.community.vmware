...
def list_or_dict(value):
    if isinstance(value, list) or isinstance(value, dict):
        ...
        raise ValueError(
            "'%s' is not valid, valid type is 'list' or 'dict'." % value
        )
...
class PyVmomiDeviceHelper(object):
    ...
    def integer_value(self, input_value, name):
        ...
        if isinstance(input_value, int):
            ...
            self.module.fail_json(
                msg='"%s" attribute should be an' " integer value." % name
            )
...
class PyVmomiCache(object):
    ...
    def find_obj(self, content, types, name, confine_to_datacenter=True):
        ...
        if result and confine_to_datacenter:
            if to_text(self.get_parent_datacenter(result).name) != to_text(
                self.dc_name
            ):
                result = None
                objects = self.get_all_objs(
                    content, types, confine_to_datacenter=True
                )
                for obj in objects:
        ...
        if confine_to_datacenter:
            if hasattr(objects, "items"):
                ...
                objects = [
                    x
                    for x in objects
                    if self.get_parent_datacenter(x).name == self.dc_name
                ]
    ...
    def get_network(self, network):
        ...
        if network not in self.networks:
            self.networks[network] = self.find_obj(
                self.content, [vim.Network], network
            )
    ...
    def get_cluster(self, cluster):
        if cluster not in self.clusters:
            self.clusters[cluster] = self.find_obj(
                self.content, [vim.ClusterComputeResource], cluster
            )
    ...
    def get_esx_host(self, host):
        if host not in self.esx_hosts:
            self.esx_hosts[host] = self.find_obj(
                self.content, [vim.HostSystem], host
            )
...
class PyVmomiHelper(PyVmomi):
    ...
    def remove_vm(self, vm, delete_from_inventory=False):
        ...
        if vm.summary.runtime.powerState.lower() == "poweredon":
            self.module.fail_json(
                msg="Virtual machine %s found in 'powered on' state, "
        ...
        if delete_from_inventory:
            try:
                vm.UnregisterVM()
            except (vim.fault.TaskInProgress, vmodl.RuntimeFault) as e:
                return {
    ...
    def configure_guestid(self, vm_obj, vm_creation=False):
        ...
        if self.params["guest_id"] and (
            vm_obj is None
            or self.params["guest_id"].lower()
            != vm_obj.summary.config.guestId.lower()
        ):
        ...
        if "hardware" in self.params:
            if "mem_limit" in self.params["hardware"]:
                ...
                try:
                    ...
                    self.module.fail_json(
                        msg="hardware.mem_limit attribute should be an integer value."
                    )
                memory_allocation.limit = mem_limit
                if (
                    vm_obj is None
                    or memory_allocation.limit
                    != vm_obj.config.memoryAllocation.limit
                ):
                    rai_change_detected = True

            if (
                "mem_reservation" in self.params["hardware"]
                or "memory_reservation" in self.params["hardware"]
            ):
                mem_reservation = self.params["hardware"].get(
                    "mem_reservation"
                )
                ...
                try:
                    ...
                    self.module.fail_json(
                        msg="hardware.mem_reservation or hardware.memory_reservation should be an integer value."
                    )

                memory_allocation.reservation = mem_reservation
                if (
                    vm_obj is None
                    or memory_allocation.reservation
                    != vm_obj.config.memoryAllocation.reservation
                ):
                    rai_change_detected = True
            ...
            if "cpu_limit" in self.params["hardware"]:
                ...
                try:
                    ...
                    self.module.fail_json(
                        msg="hardware.cpu_limit attribute should be an integer value."
                    )
                cpu_allocation.limit = cpu_limit
                if (
                    vm_obj is None
                    or cpu_allocation.limit
                    != vm_obj.config.cpuAllocation.limit
                ):
                    rai_change_detected = True
            ...
            if "cpu_reservation" in self.params["hardware"]:
                ...
                try:
                    cpu_reservation = int(
                        self.params["hardware"].get("cpu_reservation")
                    )
                except ValueError:
                    self.module.fail_json(
                        msg="hardware.cpu_reservation should be an integer value."
                    )
                cpu_allocation.reservation = cpu_reservation
                if (
                    vm_obj is None
                    or cpu_allocation.reservation
                    != vm_obj.config.cpuAllocation.reservation
                ):
                    rai_change_detected = True
        ...
        if "hardware" in self.params:
            if "num_cpus" in self.params["hardware"]:
                try:
                    ...
                    self.module.fail_json(
                        msg="hardware.num_cpus attribute should be an integer value."
                    )
                ...
                if (
                    ...
                    if (
                        not vm_obj.config.cpuHotRemoveEnabled
                        and num_cpus < vm_obj.config.hardware.numCPU
                    ):
                        self.module.fail_json(
                            msg="Configured cpu number is less than the cpu number of the VM, "
                            "cpuHotRemove is not enabled"
                        )
                    if (
                        not vm_obj.config.cpuHotAddEnabled
                        and num_cpus > vm_obj.config.hardware.numCPU
                    ):
                        self.module.fail_json(
                            msg="Configured cpu number is more than the cpu number of the VM, "
                ...
                if "num_cpu_cores_per_socket" in self.params["hardware"]:
                    try:
                        num_cpu_cores_per_socket = int(
                            self.params["hardware"]["num_cpu_cores_per_socket"]
                        )
                    except ValueError:
                        self.module.fail_json(
                            msg="hardware.num_cpu_cores_per_socket attribute "
                            "should be an integer value."
                        )
                    if num_cpus % num_cpu_cores_per_socket != 0:
                        self.module.fail_json(
                            msg="hardware.num_cpus attribute should be a multiple "
                            "of hardware.num_cpu_cores_per_socket"
                        )
                    self.configspec.numCoresPerSocket = (
                        num_cpu_cores_per_socket
                    )
                    if (
                        vm_obj is None
                        or self.configspec.numCoresPerSocket
                ...
                if (
                    vm_obj is None
                    or self.configspec.numCPUs != vm_obj.config.hardware.numCPU
                ):
                    self.change_detected = True
            ...
            if "memory_mb" in self.params["hardware"]:
                try:
                    ...
                    self.module.fail_json(
                        msg="Failed to parse hardware.memory_mb value."
                ...
                if (
                    ...
                    if (
                        vm_obj.config.memoryHotAddEnabled
                        and memory_mb < vm_obj.config.hardware.memoryMB
                    ):
                        self.module.fail_json(
                            msg="Configured memory is less than memory size of the VM, "
                            "operation is not supported"
                        )
                    elif (
                        not vm_obj.config.memoryHotAddEnabled
                        and memory_mb != vm_obj.config.hardware.memoryMB
                    ):
                        self.module.fail_json(
                            msg="memoryHotAdd is not enabled"
                        )
                self.configspec.memoryMB = memory_mb
                if (
                    vm_obj is None
                    or self.configspec.memoryMB
                    != vm_obj.config.hardware.memoryMB
                ):
                    self.change_detected = True
            # memory_mb is mandatory for VM creation
            elif vm_creation and not self.params["template"]:
                self.module.fail_json(
                    msg="hardware.memory_mb attribute is mandatory for VM creation"
                )
            ...
            if "hotadd_memory" in self.params["hardware"]:
                if (
                    ...
                    and vm_obj.runtime.powerState
                    == vim.VirtualMachinePowerState.poweredOn
                    and vm_obj.config.memoryHotAddEnabled
                    != bool(self.params["hardware"]["hotadd_memory"])
                ):
                    self.module.fail_json(
                        msg="Configure hotadd memory operation is not supported when VM is power on"
                    )
                self.configspec.memoryHotAddEnabled = bool(
                    self.params["hardware"]["hotadd_memory"]
                )
                if (
                    vm_obj is None
                    or self.configspec.memoryHotAddEnabled
                    != vm_obj.config.memoryHotAddEnabled
                ):
                    self.change_detected = True
            ...
            if "hotadd_cpu" in self.params["hardware"]:
                if (
                    ...
                    and vm_obj.runtime.powerState
                    == vim.VirtualMachinePowerState.poweredOn
                    and vm_obj.config.cpuHotAddEnabled
                    != bool(self.params["hardware"]["hotadd_cpu"])
                ):
                    self.module.fail_json(
                        msg="Configure hotadd cpu operation is not supported when VM is power on"
                    )
                self.configspec.cpuHotAddEnabled = bool(
                    self.params["hardware"]["hotadd_cpu"]
                )
                if (
                    vm_obj is None
                    or self.configspec.cpuHotAddEnabled
                    != vm_obj.config.cpuHotAddEnabled
                ):
                    self.change_detected = True
            ...
            if "hotremove_cpu" in self.params["hardware"]:
                if (
                    ...
                    and vm_obj.runtime.powerState
                    == vim.VirtualMachinePowerState.poweredOn
                    and vm_obj.config.cpuHotRemoveEnabled
                    != bool(self.params["hardware"]["hotremove_cpu"])
                ):
                    self.module.fail_json(
                        msg="Configure hotremove cpu operation is not supported when VM is power on"
                    )
                self.configspec.cpuHotRemoveEnabled = bool(
                    self.params["hardware"]["hotremove_cpu"]
                )
                if (
                    vm_obj is None
                    or self.configspec.cpuHotRemoveEnabled
                    != vm_obj.config.cpuHotRemoveEnabled
                ):
                    self.change_detected = True

            if "memory_reservation_lock" in self.params["hardware"]:
                self.configspec.memoryReservationLockedToMax = bool(
                    self.params["hardware"]["memory_reservation_lock"]
                )
                if (
                    vm_obj is None
                    or self.configspec.memoryReservationLockedToMax
                    != vm_obj.config.memoryReservationLockedToMax
                ):
                    self.change_detected = True
            ...
            if "boot_firmware" in self.params["hardware"]:
                ...
                if vm_obj is not None:
                    return
                boot_firmware = self.params["hardware"][
                    "boot_firmware"
                ].lower()
                if boot_firmware not in ("bios", "efi"):
                    self.module.fail_json(
                        msg="hardware.boot_firmware value is invalid [%s]."
    ...
    def sanitize_cdrom_params(self):
        ...
        if expected_cdrom_spec:
            for cdrom_spec in expected_cdrom_spec:
                cdrom_spec["controller_type"] = cdrom_spec.get(
                    "controller_type", "ide"
                ).lower()
                if cdrom_spec["controller_type"] not in ["ide", "sata"]:
                    self.module.fail_json(
                        msg="Invalid cdrom.controller_type: %s, valid value is 'ide' or 'sata'."
                        % cdrom_spec["controller_type"]
                    )

                cdrom_spec["state"] = cdrom_spec.get(
                    "state", "present"
                ).lower()
                if cdrom_spec["state"] not in ["present", "absent"]:
                    self.module.fail_json(
                        msg="Invalid cdrom.state: %s, valid value is 'present', 'absent'."
                        % cdrom_spec["state"]
                    )

                if cdrom_spec["state"] == "present":
                    if "type" in cdrom_spec and cdrom_spec.get("type") not in [
                        "none",
                        "client",
                        "iso",
                    ]:
                        self.module.fail_json(
                            msg="Invalid cdrom.type: %s, valid value is 'none', 'client' or 'iso'."
                            % cdrom_spec.get("type")
                        )
                    if cdrom_spec.get("type") == "iso" and not cdrom_spec.get(
                        "iso_path"
                    ):
                        self.module.fail_json(
                            msg="cdrom.iso_path is mandatory when cdrom.type is set to iso."
                        )

                if cdrom_spec["controller_type"] == "ide" and (
                    cdrom_spec.get("controller_number") not in [0, 1]
                    or cdrom_spec.get("unit_number") not in [0, 1]
                ):
                    self.module.fail_json(
                        msg="Invalid cdrom.controller_number: %s or cdrom.unit_number: %s, valid"
                        " values are 0 or 1 for IDE controller."
                        % (
                            cdrom_spec.get("controller_number"),
                            cdrom_spec.get("unit_number"),
                        )
                    )

                if cdrom_spec["controller_type"] == "sata" and (
                    cdrom_spec.get("controller_number") not in range(0, 4)
                    or cdrom_spec.get("unit_number") not in range(0, 30)
                ):
                    self.module.fail_json(
                        msg="Invalid cdrom.controller_number: %s or cdrom.unit_number: %s,"
                ...
                for exist_spec in cdroms.get(cdrom_spec["controller_type"]):
                    if exist_spec["num"] == cdrom_spec["controller_number"]:
                        ...
                        break
                if not ctl_exist:
                    cdroms.get(cdrom_spec["controller_type"]).append(
                        {
                            "num": cdrom_spec["controller_number"],
                            "cdrom": [cdrom_spec],
                        }
                    )
    ...
    def configure_cdrom_dict(self, vm_obj):
        if self.params["cdrom"].get("type") not in ["none", "client", "iso"]:
            self.module.fail_json(
                msg="cdrom.type is mandatory. Options are 'none', 'client', and 'iso'."
            )
        if self.params["cdrom"]["type"] == "iso" and not self.params[
            "cdrom"
        ].get("iso_path"):
            self.module.fail_json(
                msg="cdrom.iso_path is mandatory when cdrom.type is set to iso."
        ...
        if len(cdrom_devices) == 0:
            ...
            if len(ide_devices) == 0:
                ...
                if len(ide_device.device) > 3:
                    self.module.fail_json(
                        msg="hardware.cdrom specified for a VM or template which already has 4"
                        " IDE devices of which none are a cdrom"
                    )

            cdrom_spec = self.device_helper.create_cdrom(
                ide_device=ide_device,
                cdrom_type=self.params["cdrom"]["type"],
                iso_path=iso_path,
            )
            if (
                vm_obj
                ...
                cdrom_spec.device.connectable.connected = (
                    self.params["cdrom"]["type"] != "none"
                )

        elif not self.device_helper.is_equal_cdrom(
            vm_obj=vm_obj,
            cdrom_device=cdrom_devices[0],
            cdrom_type=self.params["cdrom"]["type"],
            iso_path=iso_path,
        ):
            self.device_helper.update_cdrom_config(
                vm_obj,
                self.params["cdrom"],
                cdrom_devices[0],
                iso_path=iso_path,
    ...
    def configure_cdrom_list(self, vm_obj):
        ...
        if configured_cdroms["ide"]:
            ...
            for expected_cdrom_spec in configured_cdroms["ide"]:
                ...
                if not ide_device:
                    ide_ctl = self.device_helper.create_ide_controller(
                        bus_number=expected_cdrom_spec["num"]
                    )
                    ide_device = ide_ctl.device
                    self.change_detected = True
                    self.configspec.deviceChange.append(ide_ctl)
                ...
                for cdrom in expected_cdrom_spec["cdrom"]:
                    ...
                    for target_cdrom in cdrom_devices:
                        if (
                            target_cdrom.controllerKey == ide_device.key
                            and target_cdrom.unitNumber == unit_number
                        ):
                    ...
                    if not cdrom_device and cdrom.get("state") != "absent":
                        if (
                            ...
                            self.module.fail_json(
                                msg="CD-ROM attach to IDE controller not support hot-add."
                            )
                        if len(ide_device.device) == 2:
                            self.module.fail_json(
                                msg="Maximum number of CD-ROMs attached to IDE controller is 2."
                            )
                        cdrom_spec = self.device_helper.create_cdrom(
                            ide_device=ide_device,
                            cdrom_type=cdrom["type"],
                            iso_path=iso_path,
                    ...
                    # re-configure CD-ROM
                        ...
                        cdrom_device
                        and cdrom.get("state") != "absent"
                        and not self.device_helper.is_equal_cdrom(
                            vm_obj=vm_obj,
                        ...
                        self.device_helper.update_cdrom_config(
                            vm_obj, cdrom, cdrom_device, iso_path=iso_path
                        )
                        cdrom_spec = vim.vm.device.VirtualDeviceSpec()
                        cdrom_spec.operation = (
                            vim.vm.device.VirtualDeviceSpec.Operation.edit
                    ...
                    # delete CD-ROM
                        ...
                        if (
                            ...
                            self.module.fail_json(
                                msg="CD-ROM attach to IDE controller not support hot-remove."
                            )
                        cdrom_spec = self.device_helper.remove_cdrom(
                            cdrom_device
                        )
                        self.change_detected = True
                        self.configspec.deviceChange.append(cdrom_spec)
    ...
    def configure_hardware_params(self, vm_obj):
        ...
        if "hardware" in self.params:
            if "max_connections" in self.params["hardware"]:
                ...
                self.configspec.maxMksConnections = int(
                    self.params["hardware"]["max_connections"]
                )
                if (
                    vm_obj is None
                    or self.configspec.maxMksConnections
                    != vm_obj.config.maxMksConnections
                ):
                    self.change_detected = True

            if "nested_virt" in self.params["hardware"]:
                self.configspec.nestedHVEnabled = bool(
                    self.params["hardware"]["nested_virt"]
                )
                if vm_obj is None or self.configspec.nestedHVEnabled != bool(
                    vm_obj.config.nestedHVEnabled
                ):
                    self.change_detected = True
            ...
            if "version" in self.params["hardware"]:
                ...
                if (
                    isinstance(temp_version, str)
                    and temp_version.lower() == "latest"
                ):
                    ...
                    if hw_version_check_failed:
                        self.module.fail_json(
                            msg="Failed to set hardware.version '%s' value as valid"
                            " values range from 3 (ESX 2.x) to 14 (ESXi 6.5 and greater)."
                            % temp_version
                        )
                    ...
                    if (
                        vm_obj is None
                        or self.configspec.version != vm_obj.config.version
                    ):
                        self.change_detected = True
                    # Check is to make sure vm_obj is not of type template
                    if vm_obj and not vm_obj.config.template:
                        ...
                        if temp_version < version_digit:
                            self.module.fail_json(
                                msg="Current hardware version '%d' which is greater than the specified"
                                " version '%d'. Downgrading hardware version is"
                                " not supported. Please specify version greater"
                                " than the current version."
                                % (version_digit, temp_version)
                            )
            ...
            if "virt_based_security" in self.params["hardware"]:
                host_version = (
                    self.select_host().summary.config.product.version
                )
                if int(host_version.split(".")[0]) < 6 or (
                    int(host_version.split(".")[0]) == 6
                    and int(host_version.split(".")[1]) < 7
                ):
                    self.module.fail_json(
                        msg="ESXi version %s not support VBS." % host_version
                    )
                ...
                if guestid not in guest_ids:
                    self.module.fail_json(
                        msg="Guest '%s' not support VBS." % guestid
                    )
                if (
                    vm_obj is None
                    and int(self.configspec.version.split("-")[1]) >= 14
                ) or (
                    vm_obj
                    and int(vm_obj.config.version.split("-")[1]) >= 14
                    and (
                        vm_obj.runtime.powerState
                        == vim.VirtualMachinePowerState.poweredOff
                    )
                    ...
                    self.configspec.flags.vbsEnabled = bool(
                        self.params["hardware"]["virt_based_security"]
                    )
                    if bool(self.params["hardware"]["virt_based_security"]):
                        ...
                        if (
                            vm_obj is None
                            and self.configspec.firmware == "efi"
                        ) or (vm_obj and vm_obj.config.firmware == "efi"):
                            self.configspec.bootOptions = vim.vm.BootOptions()
                            self.configspec.bootOptions.efiSecureBootEnabled = (
                                True
                            )
                        else:
                            self.module.fail_json(
                                msg="Not support VBS when firmware is BIOS."
                            )
                    if (
                        vm_obj is None
                        or self.configspec.flags.vbsEnabled
                        != vm_obj.config.flags.vbsEnabled
                    ):
                        self.change_detected = True
    ...
    def get_vm_ide_devices(self, vm=None):
        return self.get_device_by_type(
            vm=vm, type=vim.vm.device.VirtualIDEController
        )
    ...
    def sanitize_network_params(self):
        ...
        for network in self.params["networks"]:
            if "name" not in network and "vlan" not in network:
                self.module.fail_json(
                    msg="Please specify at least a network name or"
                    " a VLAN name under VM network list."
                )

            if (
                "name" in network
                and self.cache.get_network(network["name"]) is None
            ):
                self.module.fail_json(
                    msg="Network '%(name)s' does not exist." % network
                )
            elif "vlan" in network:
                dvps = self.cache.get_all_objs(
            ...
            if "type" in network:
                if network["type"] not in ["dhcp", "static"]:
                    self.module.fail_json(
                        msg="Network type '%(type)s' is not a valid parameter."
                        " Valid parameters are ['dhcp', 'static']." % network
                    )
                if network["type"] != "static" and (
                    "ip" in network or "netmask" in network
                ):
                    self.module.fail_json(
                        msg='Static IP information provided for network "%(name)s",'
            ...
            if (
                "device_type" in network
                and network["device_type"] not in validate_device_types
            ):
                self.module.fail_json(
                    msg="Device type specified '%s' is not valid."
            ...
            if "mac" in network and not is_mac(network["mac"]):
                self.module.fail_json(
                    msg="Device MAC address '%s' is invalid."
    ...
    def configure_network(self, vm_obj):
        ...
        if len(network_devices) < len(current_net_devices):
            self.module.fail_json(
                msg="Given network device list is lesser than current VM device list (%d < %d). "
        ...
        for key in range(0, len(network_devices)):
            ...
            if key < len(current_net_devices) and (
                vm_obj or self.params["template"]
            ):
                ...
                if "start_connected" in network_devices[
                    key
                ] and nic.device.connectable.startConnected != network_devices[
                    key
                ].get(
                    "start_connected"
                ):
                    nic.device.connectable.startConnected = network_devices[
                        key
                    ].get("start_connected")
                    nic_change_detected = True
                if "allow_guest_control" in network_devices[
                    ...
                    nic.device.connectable.allowGuestControl = network_devices[
                        key
                    ].get("allow_guest_control")
                    nic_change_detected = True
                ...
                if "device_type" in network_devices[key]:
                    device = self.device_helper.get_device(
                        network_devices[key]["device_type"], network_name
                    )
                    ...
                    if not isinstance(nic.device, device_class):
                        self.module.fail_json(
                            ...
                            "The failing device type is %s"
                            % network_devices[key]["device_type"]
                        )
                # Changing mac address has no effect when editing interface
                if (
                    "mac" in network_devices[key]
                    and nic.device.macAddress
                    != current_net_devices[key].macAddress
                ):
                    self.module.fail_json(
                        msg="Changing MAC address has not effect when interface is already present. "
                        "The failing new MAC address is %s"
                        % nic.device.macAddress
                    )

            else:
                # Default device type is vmxnet3, VMware best practice
                device_type = network_devices[key].get(
                    "device_type", "vmxnet3"
                )
                nic = self.device_helper.create_nic(
                    device_type,
            ...
            if hasattr(self.cache.get_network(network_name), "portKeys"):
                ...
                if "dvswitch_name" in network_devices[key]:
                    ...
                    if dvs_obj is None:
                        self.module.fail_json(
                            msg="Unable to find distributed virtual switch %s"
                            % dvs_name
                        )
                    pg_obj = find_dvspg_by_name(dvs_obj, network_name)
                    if pg_obj is None:
                        self.module.fail_json(
                            msg="Unable to find distributed port group %s"
                            % network_name
                        )
                else:
                    pg_obj = self.cache.find_obj(
                        self.content,
                ...
                if host_system and host_system not in [
                    host.config.host.name
                    for host in pg_obj.config.distributedVirtualSwitch.config.host
                ]:
                    self.module.fail_json(
                        msg="It seems that host system '%s' is not associated with distributed"
                        " virtual portgroup '%s'. Please make sure host system is associated"
                        " with given distributed virtual portgroup"
                        % (host_system, pg_obj.name)
                    )
                ...
                self.cache.get_network(network_name), vim.OpaqueNetwork
            ):
                # NSX-T Logical Switch
                nic.device.backing = (
                    vim.vm.device.VirtualEthernetCard.OpaqueNetworkBackingInfo()
                )
                network_id = self.cache.get_network(
                    network_name
                ).summary.opaqueNetworkId
    ...
    def configure_vapp_properties(self, vm_obj):
        ...
        for x in self.params["vapp_properties"]:
            if not x.get("id"):
                self.module.fail_json(
                    msg="id is required to set vApp property"
                )
        ...
        if vm_obj:
            ...
            orig_spec = (
                vm_obj.config.vAppConfig
                if vm_obj.config.vAppConfig
                else new_vmconfig_spec
            )

            vapp_properties_current = dict(
                (x.id, x) for x in orig_spec.property
            )
            vapp_properties_to_change = dict(
                (x["id"], x) for x in self.params["vapp_properties"]
            )
            ...
            for (
                property_id,
                property_spec,
            ) in vapp_properties_to_change.items():
                ...
                if property_id in vapp_properties_current:
                    if property_spec.get("operation") == "remove":
                        ...
                        new_vapp_property_spec.removeKey = vapp_properties_current[
                            property_id
                        ].key
                        ...
                        new_vapp_property_spec.info = vapp_properties_current[
                            property_id
                        ]
                        try:
                            for (
                            ...
                            msg = (
                                "Failed to set vApp property field='%s' and value='%s'. Error: %s"
                    ...
                    property_info.userConfigurable = property_spec.get(
                        "userConfigurable", True
                    )
                    property_info.defaultValue = property_spec.get(
                        "defaultValue"
                    )
                    property_info.value = property_spec.get("value", "")
                    property_info.description = property_spec.get(
                        "description"
                    )
    ...
    def customize_customvalues(self, vm_obj, config_spec):
        ...
        for kv in self.params["customvalues"]:
            ...
            if (
                kv["key"] not in facts["customvalues"]
                or facts["customvalues"][kv["key"]] != kv["value"]
            ):
    ...
    def customize_vm(self, vm_obj):
        ...
        if custom_spec_name:
            ...
            if cc_mgr.DoesCustomizationSpecExist(name=custom_spec_name):
                ...
                self.module.fail_json(
                    msg="Unable to find customization specification"
        ...
        for network in self.params["networks"]:
            ...
            if "domain" in network:
                ...
                guest_map.adapter.dnsDomain = self.params["customization"][
                    "domain"
                ]
            ...
            if "dns_servers" in network:
                ...
                guest_map.adapter.dnsServerList = self.params["customization"][
                    "dns_servers"
                ]
        ...
        if "dns_servers" in self.params["customization"]:
            globalip.dnsServerList = self.params["customization"][
                "dns_servers"
            ]
        ...
        if "win" in guest_id:
            ...
            default_name = "".join(
                [c for c in default_name if c not in punctuation]
            )
            ident.userData.computerName.name = str(
                self.params["customization"].get(
                    "hostname", default_name[0:15]
                )
            )
            ident.userData.fullName = str(
                self.params["customization"].get("fullname", "Administrator")
            )
            ident.userData.orgName = str(
                self.params["customization"].get("orgname", "ACME")
            )

            if "productid" in self.params["customization"]:
                ident.userData.productId = str(
                    self.params["customization"]["productid"]
                )

            ident.guiUnattended = vim.vm.customization.GuiUnattended()

            if "autologon" in self.params["customization"]:
                ident.guiUnattended.autoLogon = self.params["customization"][
                    "autologon"
                ]
                ident.guiUnattended.autoLogonCount = self.params[
                    "customization"
                ].get("autologoncount", 1)
            ...
            if self.params["customization"].get("password", "") != "":
                ...
                ident.guiUnattended.password.value = str(
                    self.params["customization"]["password"]
                )
                ident.guiUnattended.password.plainText = True

            if "joindomain" in self.params["customization"]:
                if (
                    "domainadmin" not in self.params["customization"]
                    or "domainadminpassword"
                    not in self.params["customization"]
                ):
                    self.module.fail_json(
                ...
                ident.identification.domainAdmin = str(
                    self.params["customization"]["domainadmin"]
                )
                ident.identification.joinDomain = str(
                    self.params["customization"]["joindomain"]
                )
                ident.identification.domainAdminPassword = (
                ...
                ident.identification.joinWorkgroup = str(
                    self.params["customization"]["joinworkgroup"]
                )

            if "runonce" in self.params["customization"]:
                ident.guiRunOnce = vim.vm.customization.GuiRunOnce()
                ident.guiRunOnce.commandList = self.params["customization"][
                    "runonce"
                ]
    ...
    def get_configured_disk_size(self, expected_disk_spec):
        ...
        if [
            x
            for x in expected_disk_spec.keys()
            ...
            if "size" in expected_disk_spec:
                ...
                try:
                    ...
                    # Common failure
                    self.module.fail_json(
                        msg="Failed to parse disk size please review value"
                ...
                expected = [
                    x[1]
                    for x in expected_disk_spec.items()
                    if x[0].startswith("size_")
                ][0]
                expected = int(expected)
            ...
            if unit in disk_units:
                ...
                self.module.fail_json(
                    msg="%s is not a supported unit for disk size."
                    " Supported units are ['%s']."
                    % (unit, "', '".join(disk_units.keys()))
                )
    ...
    def add_existing_vmdk(
        self, vm_obj, expected_disk_spec, diskspec, scsi_ctl
    ):
        """
        ...
        if (
            vm_obj and diskspec.device.backing.fileName != filename
        ) or vm_obj is None:
    ...
    def configure_disks(self, vm_obj):
        ...
        disks = (
            [
                x
                for x in vm_obj.config.hardware.device
        ...
        if (
            disks is not None
            and self.params.get("disk")
            and len(self.params.get("disk")) < len(disks)
        ):
            self.module.fail_json(
                msg="Provided disks configuration has less disks than "
                "the target object (%d vs %d)"
                % (len(self.params.get("disk")), len(disks))
            )
        ...
        for expected_disk_spec in self.params.get("disk"):
            ...
            if (
                vm_obj is not None
                and disks is not None
                and disk_index < len(disks)
            ):
                diskspec = vim.vm.device.VirtualDeviceSpec()
                ...
                diskspec = self.device_helper.create_scsi_disk(
                    scsi_ctl, disk_index
                )
                disk_modified = True
            ...
            if "disk_mode" in expected_disk_spec:
                disk_mode = expected_disk_spec.get(
                ...
                if disk_mode not in valid_disk_mode:
                    self.module.fail_json(
                        msg="disk_mode specified is not valid."
                        " Should be one of ['%s']"
                        % "', '".join(valid_disk_mode)
                    )

                if (
                    vm_obj and diskspec.device.backing.diskMode != disk_mode
                ) or (vm_obj is None):
            ...
            if (
                "filename" in expected_disk_spec
                and expected_disk_spec["filename"] is not None
            ):
                self.add_existing_vmdk(
                    vm_obj, expected_disk_spec, diskspec, scsi_ctl
                )
                continue
    ...
    def select_host(self):
        ...
        if not hostsystem:
            self.module.fail_json(
                msg='Failed to find ESX host "%(esxi_hostname)s"' % self.params
            )
        if (
            hostsystem.runtime.connectionState != "connected"
            or hostsystem.runtime.inMaintenanceMode
        ):
            self.module.fail_json(
                msg='ESXi "%(esxi_hostname)s" is in invalid state or in maintenance mode.'
                % self.params
            )
        return hostsystem
    ...
    def autoselect_datastore(self):
        ...
        if datastores is None or len(datastores) == 0:
            self.module.fail_json(
                msg="Unable to find a datastore list when autoselecting"
            )
    ...
    def select_datastore(self, vm_obj=None):
        ...
        if len(self.params["disk"]) != 0:
            ...
            if (
                "autoselect_datastore" in self.params["disk"][0]
                and self.params["disk"][0]["autoselect_datastore"]
            ):
                datastores = []

                if self.params["cluster"]:
                    cluster = self.find_cluster_by_name(
                        self.params["cluster"], self.content
                    )
                    ...
                    datastores = self.cache.get_all_objs(
                        self.content, [vim.Datastore]
                    )
                    datastores = [
                        x
                        for x in datastores
                        if self.cache.get_parent_datacenter(x).name
                ...
                for ds in datastores:
                    ...
                    if (ds.summary.freeSpace > datastore_freespace) or (
                        ...
                        # If datastore field is provided, filter destination datastores
                        if (
                            "datastore" in self.params["disk"][0]
                            and isinstance(
                                self.params["disk"][0]["datastore"], str
                            )
                            and ds.name.find(
                                self.params["disk"][0]["datastore"]
                            )
                            < 0
                        ):
                            continue
                ...
                datastore_cluster = self.cache.find_obj(
        ...
        if not datastore:
            if (
                len(self.params["disk"]) != 0
                or self.params["template"] is None
            ):
                self.module.fail_json(
                    msg="Unable to find the datastore with given parameters."
    ...
    def get_scsi_type(self):
        ...
        if "hardware" in self.params:
            if "scsi" in self.params["hardware"]:
                if self.params["hardware"]["scsi"] in [
                    "buslogic",
                    "paravirtual",
                    "lsilogic",
                    "lsilogicsas",
                    ...
                    self.module.fail_json(
                        msg="hardware.scsi attribute should be 'paravirtual' or 'lsilogic'"
                    )
    ...
    def get_resource_pool(self, cluster=None, host=None, resource_pool=None):
        ...
        resource_pool_name = resource_pool or self.params.get(
            "resource_pool", None
        )

        # get the datacenter object
        datacenter = find_obj(
            self.content, [vim.Datacenter], self.params["datacenter"]
        )
        if not datacenter:
            self.module.fail_json(
                msg='Unable to find datacenter "%s"'
                % self.params["datacenter"]
            )
        ...
        if cluster_name:
            ...
            if not cluster:
                self.module.fail_json(
                    msg='Unable to find cluster "%s"' % cluster_name
                )
        # if host is given, get the cluster object using the host
        elif host_name:
            host = find_obj(
                self.content, [vim.HostSystem], host_name, folder=datacenter
            )
            if not host:
                self.module.fail_json(
                    msg='Unable to find host "%s"' % host_name
                )
            cluster = host.parent
        else:
            cluster = None
        ...
        resource_pool = find_obj(
            self.content,
            [vim.ResourcePool],
            resource_pool_name,
            folder=cluster or datacenter,
        )
        if not resource_pool:
            if resource_pool_name:
                self.module.fail_json(
                    msg='Unable to find resource_pool "%s"'
                    % resource_pool_name
                )
            else:
                self.module.fail_json(
                    msg="Unable to find resource pool, need esxi_hostname, resource_pool, or cluster"
                )
    ...
    def deploy_vm(self):
        ...
        datacenter = self.cache.find_obj(
            self.content, [vim.Datacenter], self.params["datacenter"]
        )
        if datacenter is None:
            self.module.fail_json(
                msg="No datacenter named %(datacenter)s was found"
                % self.params
            )
        ...
        if self.params["template"]:
            vm_obj = self.get_vm_or_template(
                template_name=self.params["template"]
            )
            if vm_obj is None:
                self.module.fail_json(
                    msg="Could not find a template named %(template)s"
                    % self.params
                )
        else:
            vm_obj = None
        ...
        if (
            len(self.params["customization"]) > 0
            or network_changes
            or self.params.get("customization_spec") is not None
        ):
        ...
        try:
            if self.params["template"]:
                    ...
                    for device in vm_obj.config.hardware.device:
                        if isinstance(device, vim.vm.device.VirtualDisk):
                            ...
                            if self.params["convert"] in ["thin"]:
                                disk_locator.diskBackingInfo.thinProvisioned = (
                                    True
                                )
                            if self.params["convert"] in ["eagerzeroedthick"]:
                                disk_locator.diskBackingInfo.eagerlyScrub = (
                                    True
                                )
                            if self.params["convert"] in ["thick"]:
                                disk_locator.diskBackingInfo.diskMode = (
                                    "persistent"
                                )
                ...
                if linked_clone:
                    if snapshot_src is not None:
                        self.relospec.diskMoveType = (
                            vim.vm.RelocateSpec.DiskMoveOptions.createNewChildDiskBacking
                        )
                    else:
                        self.module.fail_json(
                            msg="Parameter 'linked_src' and 'snapshot_src' are"
                            " required together for linked clone operation."
                        )

                clonespec = vim.vm.CloneSpec(
                    template=self.params["is_template"], location=self.relospec
                )
                if self.customspec:
                ...
                if snapshot_src is not None:
                    if vm_obj.snapshot is None:
                        self.module.fail_json(
                            msg="No snapshots present for virtual machine or template [%(template)s]"
                            % self.params
                        )
                    snapshot = self.get_snapshots_by_name_recursively(
                        snapshots=vm_obj.snapshot.rootSnapshotList,
                        snapname=snapshot_src,
                    )
                    if len(snapshot) != 1:
                        self.module.fail_json(
                            msg='virtual machine "%(template)s" does not contain'
                ...
                try:
                    ...
                    self.module.fail_json(
                        msg="Failed to clone virtual machine %s to folder %s "
                        "due to permission issue: %s"
                ...
                self.configspec.files = vim.vm.FileInfo(
                    logDirectory=None,
                ...
                try:
                    task = destfolder.CreateVM_Task(
                        config=self.configspec, pool=resource_pool
                    )
                except vmodl.fault.InvalidRequest as e:
                    self.module.fail_json(
                        msg="Failed to create virtual machine due to invalid configuration "
                        "parameter %s" % to_native(e.msg)
                    )
                except vim.fault.RestrictedVersion as e:
                    self.module.fail_json(
                        msg="Failed to create virtual machine due to "
                        "product versioning restrictions: %s"
                        % to_native(e.msg)
                    )
                self.change_detected = True
            self.wait_for_task(task)
            ...
            self.module.fail_json(
                msg="TypeError was returned, please ensure to give correct inputs. %s"
                % to_text(e)
            )
        ...
        if task.info.state == "error":
            ...
            if self.params["customvalues"]:
                ...
                self.customize_customvalues(
                    vm_obj=vm, config_spec=vm_custom_spec
                )
            ...
            if (
                self.params["wait_for_ip_address"]
                or self.params["wait_for_customization"]
                or self.params["state"] in ["poweredon", "restarted"]
            ):
                ...
                if self.params["wait_for_ip_address"]:
                    wait_for_vm_ip(
                        self.content,
                        vm,
                        self.params["wait_for_ip_address_timeout"],
                    )

                if self.params["wait_for_customization"]:
                    is_customization_ok = self.wait_for_customization(
                        vm=vm,
                        timeout=self.params["wait_for_customization_timeout"],
                    )
    ...
    def reconfigure_vm(self):
        ...
        self.customize_customvalues(
            vm_obj=self.current_vm_obj, config_spec=self.configspec
        )
        ...
        if (
            ...
            and self.current_vm_obj.config.annotation
            != self.params["annotation"]
        ):
        ...
        if self.change_detected:
            task = None
            try:
                task = self.current_vm_obj.ReconfigVM_Task(
        ...
        if (
            ...
            and self.params["name"] != self.current_vm_obj.config.name
        ):
        ...
        # Mark VM as Template
        if (
            self.params["is_template"]
            and not self.current_vm_obj.config.template
        ):
            try:
                ...
                self.module.fail_json(
                    msg="Failed to mark virtual machine [%s] "
                    "as template: %s" % (self.params["name"], e.msg)
                )

        # Mark Template as VM
        elif (
            not self.params["is_template"]
            and self.current_vm_obj.config.template
        ):
            resource_pool = self.get_resource_pool()
            ...
            try:
                ...
                self.module.fail_json(
                    msg="Failed to convert template to virtual machine"
            ...
            uuid_action = [
                x
                for x in self.current_vm_obj.config.extraConfig
                if x.key == "uuid.action"
            ]
            if not uuid_action:
        ...
        if (
            "existing_vm" in self.params["customization"]
            and self.params["customization"]["existing_vm"]
        ):
            if self.current_vm_obj.config.template:
                self.module.fail_json(
                    msg="VM is template, not support guest OS customization."
                )
            if (
                ...
                self.module.fail_json(
                    msg="VM is not in poweroff state, can not do guest OS customization."
                )
    ...
    def customize_exist_vm(self):
        ...
        for nw in self.params["networks"]:
            for key in nw:
                ...
                if key not in (
                    ...
                    break
        if (
            len(self.params["customization"]) > 1
            or network_changes
            or self.params.get("customization_spec")
        ):
        ...
        try:
            ...
            self.module.fail_json(
                msg="Failed to customization virtual machine due to CustomizationFault: %s"
                % to_native(e.msg)
            )
        except vim.fault.RuntimeFault as e:
            self.module.fail_json(
                msg="failed to customization virtual machine due to RuntimeFault: %s"
                % to_native(e.msg)
            )
        except Exception as e:
            self.module.fail_json(
                msg="failed to customization virtual machine due to fault: %s"
                % to_native(e.msg)
            )
        self.wait_for_task(task)
        if task.info.state == "error":
        ...
        if self.params["wait_for_customization"]:
            set_vm_power_state(
                self.content, self.current_vm_obj, "poweredon", force=False
            )
            is_customization_ok = self.wait_for_customization(
    ...
    def wait_for_task(self, task, poll_interval=1):
        ...
        self.change_applied = (
            self.change_applied or task.info.state == "success"
        )

    def get_vm_events(self, vm, eventTypeIdList):
        byEntity = vim.event.EventFilterSpec.ByEntity(
            entity=vm, recursion="self"
        )
        filterSpec = vim.event.EventFilterSpec(
            entity=byEntity, eventTypeId=eventTypeIdList
        )
    ...
    def wait_for_customization(self, vm, timeout=3600, sleep=10):
        ...
        thispoll = 0
        while thispoll <= poll:
            eventStarted = self.get_vm_events(
                vm, ["CustomizationStartedEvent"]
            )
            if len(eventStarted):
                thispoll = 0
                while thispoll <= poll:
                    eventsFinishedResult = self.get_vm_events(
                        vm, ["CustomizationSucceeded", "CustomizationFailed"]
                    )
                    if len(eventsFinishedResult):
                        if not isinstance(
                            eventsFinishedResult[0],
                            vim.event.CustomizationSucceeded,
                        ):
                            self.module.warn(
                                "Customization failed with error {%s}:{%s}"
                                % (
                                    eventsFinishedResult[0]._wsdlName,
                                    eventsFinishedResult[
                                        0
        ...
        if len(eventStarted):
            ...
            self.module.warn(
                "Waiting for customization start event timed out."
            )
        return False
...
def main():
    ...
    argument_spec.update(
        ...
        convert=dict(
            type="str", choices=["thin", "thick", "eagerzeroedthick"]
        ),
    ...
    if vm:
        if module.params["state"] == "absent":
            ...
            if module.params["force"]:
                ...
                set_vm_power_state(
                    pyv.content, vm, "poweredoff", module.params["force"]
                )
            ...
            if tmp_result["changed"]:
                ...
                if (
                    module.params["state"]
                    in ["poweredon", "restarted", "rebootguest"]
                    and module.params["wait_for_ip_address"]
                ):
                    wait_result = wait_for_vm_ip(
                        pyv.content,
                        vm,
                        module.params["wait_for_ip_address_timeout"],
                    )
                    if not wait_result:
                        module.fail_json(
    ...
    # VM doesn't exist
        ...
        if module.params["state"] in [
            ...
            if result["failed"]:
                module.fail_json(
                    msg="Failed to create a virtual machine : %s"
                    % result["msg"]
                )
