...
class VMwareConfigurationBackup(PyVmomi):
    ...
    def find_host_system(self):
        if self.esxi_hostname:
            host_system_obj = self.find_hostsystem_by_name(
                host_name=self.esxi_hostname
            )
            if host_system_obj:
                return host_system_obj
            else:
                self.module.fail_json(
                    msg="Failed to find ESXi %s" % self.esxi_hostname
                )
    ...
    def load_configuration(self):
        ...
        url = (
            self.host.configManager.firmwareSystem.QueryFirmwareConfigUploadURL()
        )
        ...
        try:
            request = open_url(
                url=url, method="HEAD", validate_certs=self.validate_certs
            )
    ...
    def save_configuration(self):
        url = (
            self.host.configManager.firmwareSystem.BackupFirmwareConfiguration()
        )
        url = url.replace("*", self.host.name)
        ...
        try:
            ...
            self.module.fail_json(
                msg="Failed to write backup file. Ensure that "
                "the dest path exists and is writable. Details : %s"
                % to_native(e)
            )
        except Exception as e:
            self.module.fail_json(msg=to_native(e))
...
def main():
    ...
    required_if = [("state", "saved", ["dest"]), ("state", "loaded", ["src"])]
