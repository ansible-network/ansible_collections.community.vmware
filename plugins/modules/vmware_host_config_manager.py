    ...
    def set_host_configuration_facts(self):
        ...
        for host in self.hosts:
            ...
            for s_option in option_manager.supportedOption:
                host_facts[s_option.key] = dict(
                    option_type=s_option.optionType, value=None
                )
            ...
            for option_key, option_value in self.options.items():
                if option_key in host_facts:
                    ...
                    if is_boolean(option_value) and isinstance(
                        option_type, vim.option.BoolOption
                    ):
                        option_value = is_truthy(option_value)
                    elif (
                        isinstance(option_value, integer_types)
                        or is_integer(option_value)
                    ) and isinstance(option_type, vim.option.IntOption):
                        option_value = VmomiSupport.vmodlTypes["int"](
                            option_value
                        )
                    elif (
                        isinstance(option_value, integer_types)
                        or is_integer(option_value, "long")
                    ) and isinstance(option_type, vim.option.LongOption):
                        option_value = VmomiSupport.vmodlTypes["long"](
                            option_value
                        )
                    elif isinstance(option_value, float) and isinstance(
                        option_type, vim.option.FloatOption
                    ):
                        pass
                    elif isinstance(option_value, string_types) and isinstance(
                        option_type,
                        (vim.option.StringOption, vim.option.ChoiceOption),
                    ):
                        pass
                    else:
                        self.module.fail_json(
                            msg="Provided value is of type %s."
                            " Option %s expects: %s"
                            % (
                                type(option_value),
                                option_key,
                    ...
                    self.module.fail_json(
                        msg="Unsupported option %s" % option_key
                    )
            if change_option_list:
                ...
                if self.module.check_mode is False:
                    try:
                        ...
                        self.module.fail_json(
                            msg="Failed to update option/s as one or more OptionValue "
                            "contains an invalid value: %s" % to_native(e.msg)
                        )
                    except vim.fault.InvalidName as e:
                        self.module.fail_json(
                            msg="Failed to update option/s as one or more OptionValue "
                            "objects refers to a non-existent option : %s"
                            % to_native(e.msg)
                        )
            else:
