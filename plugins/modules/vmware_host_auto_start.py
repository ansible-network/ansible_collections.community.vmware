...
class VMwareHostAutoStartManager(PyVmomi):
    ...
    def generate_system_defaults_config(self):
        ...
        system_defaults_config.waitForHeartbeat = self.system_defaults[
            "wait_for_heartbeat"
        ]
    ...
    def generate_power_info_config(self):
        ...
        power_info_config.waitForHeartbeat = self.power_info[
            "wait_for_heartbeat"
        ]
    ...
    def execute(self):
        ...
        if self.name or self.uuid or self.moid:
            ...
            if not self.vm_obj:
                self.module.fail_json(
                    msg="Cannot find the specified VM: %s"
                    % (self.name or self.uuid or self.moid)
                )
            elif self.esxi_hostname != self.vm_obj.runtime.host.name:
                self.module.fail_json(
                    msg="%s exists on another host: %s"
                    % (
                        self.name or self.uuid or self.moid,
                        self.vm_obj.runtime.host.name,
                    )
                )

        # Check the existing autoStart setting difference.
        system_defaults_config_difference = False
        existing_system_defaults = self.to_json(
            host_obj.config.autoStart.defaults
        )
        ...
        if self.system_defaults:
            ...
            if system_defaults_for_compare != self.system_defaults:
                result["diff"]["before"]["system_defaults"] = OrderedDict(
                    sorted(system_defaults_for_compare.items())
                )
                result["diff"]["after"]["system_defaults"] = OrderedDict(
                    sorted(self.system_defaults.items())
                )
        ...
        if system_defaults_for_compare["enabled"] and self.vm_obj:
            ...
            if existing_vm_power_info:
                vm_power_info_for_compare = dict(
                    ...
                    wait_for_heartbeat=existing_vm_power_info[
                        "waitForHeartbeat"
                    ],
                )
            else:
            ...
            if vm_power_info_for_compare != self.power_info:
                result["diff"]["before"]["power_info"] = OrderedDict(
                    sorted(vm_power_info_for_compare.items())
                )
                result["diff"]["after"]["power_info"] = OrderedDict(
                    sorted(self.power_info.items())
                )
        ...
        if (
            system_defaults_config_difference
            or vm_power_info_config_difference
        ):
            if system_defaults_config_difference:
                auto_start_manager_config.defaults = (
                    self.generate_system_defaults_config()
                )
                result["system_defaults_config"] = self.system_defaults

            if vm_power_info_config_difference:
                auto_start_manager_config.powerInfo = [
                    self.generate_power_info_config()
                ]
            ...
            try:
                host_obj.configManager.autoStartManager.ReconfigureAutostart(
                    spec=auto_start_manager_config
                )
                result["changed"] = True
                self.module.exit_json(**result)
...
def main():
    ...
    argument_spec.update(
        ...
        power_info=dict(
            ...
            options=dict(
                start_action=dict(
                    type="str", choices=["none", "powerOn"], default="none"
                ),
