...
class VMwareLocalRoleManager(PyVmomi):
    ...
    def process_state(self):
        ...
        try:
            local_role_manager_states[self.state][
                self.check_local_role_manager_state()
            ]()
        ...
        if self.module.check_mode:
            ...
            try:
                role_id = self.content.authorizationManager.AddAuthorizationRole(
                    name=self.role_name, privIds=self.priv_ids
                )
        ...
        if self.module.check_mode:
            ...
            try:
                self.content.authorizationManager.RemoveAuthorizationRole(
                    roleId=self.current_role.roleId, failIfUsed=self.force
                )
                ...
                self.module.fail_json(
                    ...
                    % self.role_name,
                    details=not_found.msg,
                )
            except vim.fault.RemoveFailed as remove_failed:
                msg = (
                    "Failed to remove role '%s' as the user specified role name."
                    % self.role_name
                )
                if self.force:
    ...
    def state_update_role(self):
        ...
        if self.action == "add":
            ...
            self.params["local_privilege_ids"].extend(
                ["System.Anonymous", "System.Read", "System.View"]
            )
            changed_privileges = self.params["local_privilege_ids"]
            changes_applied = list(
                set(current_privileges) ^ set(changed_privileges)
            )
            if changes_applied:
                changed = True
        ...
        if changed:
            ...
            if self.module.check_mode:
                ...
                try:
                    ...
                    self.module.fail_json(
                        msg="Failed to update role. Please check privileges provided for update",
                        details=not_found.msg,
                    )
                except vim.fault.InvalidName as invalid_name:
                    self.module.fail_json(
                        msg="Failed to update role as role name is empty",
                        details=invalid_name.msg,
                    )
                except vim.fault.AlreadyExists as already_exists:
                    self.module.fail_json(
...
def main():
    ...
    argument_spec.update(
        dict(
            local_role_name=dict(required=True, type="str"),
            local_privilege_ids=dict(default=[], type="list"),
            force_remove=dict(default=False, type="bool"),
            action=dict(
                type="str", default="set", choices=["add", "set", "remove"]
            ),
            state=dict(
