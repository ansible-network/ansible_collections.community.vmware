...
class VmwareContentDeployTemplate(VmwareRestClient):
    def __init__(self, module):
        ...
        self.template_service = (
            self.api_client.vcenter.vm_template.LibraryItems
        )
    ...
    def deploy_vm_from_template(self, power_on=False):
        ...
        if not self.datacenter_id:
            self.module.fail_json(
                msg="Failed to find the datacenter %s" % self.datacenter
            )
        ...
        if not self.datastore_id:
            self.module.fail_json(
                msg="Failed to find the datastore %s" % self.datastore
            )
        ...
        if not self.folder_id:
            self.module.fail_json(
                msg="Failed to find the folder %s" % self.folder
            )
        ...
        if self.cluster:
            self.cluster_id = self.get_cluster_by_name(
                self.datacenter, self.cluster
            )
            if not self.cluster_id:
                self.module.fail_json(
                    msg="Failed to find the Cluster %s" % self.cluster
                )
        ...
        vm_id = self.template_service.deploy(
            self.library_item_id, self.deploy_spec
        )
        if vm_id:
