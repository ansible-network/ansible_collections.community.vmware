    ...
    def run(self):
        while self._running:
            try:
                ...
                while True:
                    if (
                        self.httpNfcLease.state == vim.HttpNfcLease.State.done
                        or self.httpNfcLease.state
...
class VMwareExportVmOvf(PyVmomi):
        ...
        if not os.path.exists(self.ovf_dir):
            try:
                ...
                self.module.fail_json(
                    msg="Exception caught when create folder %s, with error %s"
        ...
        with open(self.mf_file, "a") as mf_handle:
            with open(temp_target_disk, "wb") as handle:
                try:
                    ...
                    lease_updater.stop()
                    self.module.fail_json(
                        msg="Exception caught when getting %s, %s"
                        % (device_url, to_text(err))
                    )
                if not response:
                    lease_updater.httpNfcLease.HttpNfcLeaseAbort()
                    lease_updater.stop()
                ...
                if response.getcode() >= 400:
                    ...
                    lease_updater.stop()
                    self.module.fail_json(
                        msg="Getting %s return code %d"
                        % (device_url, response.getcode())
                    )
    ...
    def export_to_ovf_files(self, vm_obj):
        ...
        if (
            "export_with_images" in self.params
            and self.params["export_with_images"]
        ):
        ...
        lease_updater = LeaseProgressUpdater(
            http_nfc_lease, self.lease_interval
        )
        ...
        if total_bytes_to_write == 0:
            ...
            if total_bytes_to_write == 0:
                ...
                self.module.fail_json(
                    msg="Total storage space occupied by the VM is 0."
                )
        ...
        try:
                ...
                if http_nfc_lease.state == vim.HttpNfcLease.State.ready:
                    for deviceUrl in http_nfc_lease.info.deviceUrl:
                        ...
                        if deviceUrl.targetId and deviceUrl.disk:
                            ...
                            if self.host_version_at_least(
                                version=(6, 7, 0), vm_obj=vm_obj
                            ):
                                file_download = True
                        else:
                            continue
                        ...
                        if device_file_name.split(".")[0][0:5] == "disk-":
                            device_file_name = device_file_name.replace(
                                "disk", vm_obj.name
                            )
                        temp_target_disk = os.path.join(
                            self.ovf_dir, device_file_name
                        )
                        ...
                        if "*" in device_url:
                            device_url = device_url.replace(
                                "*", self.params["hostname"]
                            )
                        if file_download:
                            current_bytes_written = self.download_device_files(
                                headers=headers,
                    ...
                    http_nfc_lease.state == vim.HttpNfcLease.State.initializing
                ):
                    sleep(2)
                    continue
                    ...
                    lease_updater.stop()
                    self.module.fail_json(
                        msg="Get HTTP NFC lease error %s."
                        % http_nfc_lease.state.error[0].fault
                    )
            ...
            if vm_descriptor_result.error:
                ...
                lease_updater.stop()
                self.module.fail_json(
                    msg="Create VM descriptor file error %s."
                    % vm_descriptor_result.error
                )
            else:
                vm_descriptor = vm_descriptor_result.ovfDescriptor
                ovf_descriptor_path = os.path.join(
                    self.ovf_dir, ovf_descriptor_name + ".ovf"
                )
                sha256_hash = hashlib.sha256()
                with open(self.mf_file, "a") as mf_handle:
                    ...
                    mf_handle.write(
                        "SHA256("
                        + os.path.basename(ovf_descriptor_path)
                        + ")= "
                ...
                lease_updater.stop()
                self.facts.update(
                    {"manifest": self.mf_file, "ovf_file": ovf_descriptor_path}
                )
        except Exception as err:
            kwargs = {
                "changed": False,
