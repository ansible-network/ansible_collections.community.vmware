...
def main():
    ...
    result = dict(path=path, size=None, state=state, status=None, url=url)
    ...
    try:
        ...
        module.fail_json(
            msg=to_native(e), errno=e[0], reason=to_native(e), **result
        )
    except Exception as e:
        module.fail_json(
            msg=to_native(e), errno=dir(e), reason=to_native(e), **result
        )
    ...
    if status == 200:
        ...
        module.fail_json(
            msg="Failed to query for file '%s'" % path,
            errno=None,
    ...
    if state == "absent":
        ...
        if module.check_mode:
            ...
            if result["status"] == 405:
                ...
                module.fail_json(
                    msg="Directories cannot be removed with this module",
                    errno=None,
                    headers=dict(r.headers),
                    **result
                )
    ...
    # NOTE: Creating a file in a non-existing directory, then remove the file
        ...
        if module.check_mode:
            ...
            remote_path = vmware_path(
                datastore, datacenter, path + "/foobar.tmp"
            )
            ...
            if status != 204:
                ...
                result["status"] = status
                module.warn(
                    "Failed to remove temporary file ({reason})".format(
                        **result
                    )
                )
        ...
        if not exists:
            ...
            result["status"] = status
            module.fail_json(
                msg="File '%s' is absent, cannot continue" % path, **result
            )
