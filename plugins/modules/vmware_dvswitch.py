...
class VMwareDvSwitch(PyVmomi):
    ...
    def __init__(self, module):
        ...
        if folder:
            self.folder_obj = self.content.searchIndex.FindByInventoryPath(
                folder
            )
            if not self.folder_obj:
                self.module.fail_json(
                    msg="Failed to find the folder specified by %(folder)s"
                    % self.params
                )
        else:
            ...
            if not datacenter_obj:
                self.module.fail_json(
                    msg="Failed to find datacenter '%s' required"
        ...
        if not 1280 <= self.mtu <= 9000:
            self.module.fail_json(
                msg="MTU value should be between 1280 and 9000 (both inclusive), provided %d."
                % self.mtu
            )
        self.multicast_filtering_mode = self.module.params[
            "multicast_filtering_mode"
        ]
        ...
        self.health_check_vlan_interval = self.params["health_check"].get(
            "vlan_mtu_interval"
        )
        self.health_check_teaming = self.params["health_check"].get(
            "teaming_failover"
        )
        self.health_check_teaming_interval = self.params["health_check"].get(
    ...
    def check_dvs(self):
        ...
        self.dvs = find_dvs_by_name(
            self.content, self.switch_name, folder=self.folder_obj
        )
        if self.dvs is None:
            return "absent"
        return "present"
    ...
    def create_dvswitch(self):
        ...
        if self.module.check_mode:
            ...
            if (
                self.dvs.config.multicastFilteringMode
                != multicast_filtering_mode
            ):
    ...
    def check_health_check_config(self, health_check_config):
        ...
        changed = (
            ...
            changed_vlan_interval
        ) = changed_teaming = changed_teaming_interval = False
        ...
        for config in health_check_config:
            if isinstance(
                ...
                if (
                    config.enable
                    and config.interval != self.health_check_vlan_interval
                ):
            ...
            if isinstance(
                ...
                if (
                    config.enable
                    and config.interval != self.health_check_teaming_interval
                ):
        ...
        return (
            ...
            vlan_previous,
            changed_vlan_interval,
            vlan_interval_previous,
            changed_teaming,
            teaming_previous,
            changed_teaming_interval,
            teaming_interval_previous,
        )

    def update_health_check_config(self, switch_object, health_check_config):
        """Update Health Check config"""
        try:
            task = switch_object.UpdateDVSHealthCheckConfig_Task(
                healthCheckConfig=health_check_config
            )
        except vim.fault.DvsFault as dvs_fault:
            self.module.fail_json(
                msg="Update failed due to DVS fault : %s"
                % to_native(dvs_fault)
            )
        except vmodl.fault.NotSupported as not_supported:
            self.module.fail_json(
                msg="Health check not supported on the switch : %s"
                % to_native(not_supported)
            )
        except TaskError as invalid_argument:
            self.module.fail_json(
                msg="Failed to configure health check : %s"
                % to_native(invalid_argument)
            )
        try:
            wait_for_task(task)
            ...
            self.module.fail_json(
                msg="Failed to update health check config : %s"
                % to_native(invalid_argument)
            )
    ...
    def destroy_dvswitch(self):
        ...
        if self.module.check_mode:
            ...
            try:
                ...
                self.module.fail_json(
                    msg="Failed to deleted DVS : %s" % to_native(vim_fault)
                )
            wait_for_task(task)
    ...
    def update_dvswitch(self):
        ...
        changed = (
            changed_settings
        ) = changed_ldp = changed_version = changed_health_check = False
        results = dict(changed=changed)
        results["dvswitch"] = self.switch_name
        changed_list = []
        ...
        if self.discovery_protocol == "disabled":
            ...
            results["discovery_operation"] = self.discovery_operation
            if (
                ldp_protocol != self.discovery_protocol
                or ldp_operation != self.discovery_operation
            ):
                changed_ldp = True
        ...
        if self.dvs.config.multicastFilteringMode != multicast_filtering_mode:
            ...
            results[
                "multicast_filtering_mode_previous"
            ] = self.dvs.config.multicastFilteringMode
        ...
        results["contact_details"] = self.contact_details
        if (
            self.dvs.config.contact.name != self.contact_name
            or self.dvs.config.contact.contact != self.contact_details
        ):
            changed = changed_settings = True
            changed_list.append("contact")
            results["contact_previous"] = self.dvs.config.contact.name
            results[
                "contact_details_previous"
            ] = self.dvs.config.contact.contact
        ...
        results["uplink_quantity"] = self.uplink_quantity
        if (
            len(self.dvs.config.uplinkPortPolicy.uplinkPortName)
        ...
        results[
            "health_check_teaming_interval"
        ] = self.health_check_teaming_interval
        (
            health_check_config,
            changed_health_check,
            changed_vlan,
            vlan_previous,
            changed_vlan_interval,
            vlan_interval_previous,
        ...
        if self.switch_version:
            ...
            if (
                self.dvs.config.productInfo.version
        ...
        if changed:
            ...
            if not self.module.check_mode:
                ...
                if changed_health_check:
                    self.update_health_check_config(
                        self.dvs, health_check_config
                    )
                if changed_version:
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            multicast_filtering_mode=dict(
                type="str", default="basic", choices=["basic", "snooping"]
            ),
            switch_version=dict(
                choices=["5.0.0", "5.1.0", "5.5.0", "6.0.0", "6.5.0", "6.6.0"],
                aliases=["version"],
    ...
    module = AnsibleModule(
        ...
        required_if=[("state", "present", ["uplink_quantity"])],
        required_one_of=[["folder", "datacenter_name"]],
        mutually_exclusive=[["folder", "datacenter_name"]],
