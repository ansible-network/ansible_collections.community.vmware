...
class VMwareObjectRolePermission(PyVmomi):
    ...
    def get_perms(self):
        self.current_perms = self.content.authorizationManager.RetrieveEntityPermissions(
            self.current_obj, False
        )

    def same_permission(self, perm_one, perm_two):
        return (
            perm_one.principal.lower() == perm_two.principal.lower()
    ...
    def add_permission(self):
        if not self.module.check_mode:
            self.content.authorizationManager.SetEntityPermissions(
                self.current_obj, [self.perm]
            )
        self.module.exit_json(changed=True)

    def remove_permission(self):
        if not self.module.check_mode:
            self.content.authorizationManager.RemoveEntityPermission(
                self.current_obj, self.applied_to, self.is_group
            )
        self.module.exit_json(changed=True)
    ...
    def get_role(self):
        ...
        self.module.fail_json(
            msg="Specified role (%s) was not found" % self.params["role"]
        )
