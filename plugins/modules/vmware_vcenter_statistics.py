...
class VmwareVcenterStatistics(PyVmomi):
    ...
    def ensure(self):
        ...
        past_day_enabled = self.params["interval_past_day"].get(
            "enabled", True
        )
        past_day_seconds = (
            self.params["interval_past_day"].get("interval_minutes", 5) * 60
        )
        past_day_save_for_seconds = (
            self.params["interval_past_day"].get("save_for_days", 1) * 86400
        )
        past_day_level = self.params["interval_past_day"].get("level", 1)
        past_week_enabled = self.params["interval_past_week"].get(
            "enabled", True
        )
        past_week_seconds = (
            self.params["interval_past_week"].get("interval_minutes", 30) * 60
        )
        past_week_save_for_seconds = (
            self.params["interval_past_week"].get("save_for_weeks", 1) * 604800
        )
        past_week_level = self.params["interval_past_week"].get("level", 1)
        past_month_enabled = self.params["interval_past_month"].get(
            "enabled", True
        )
        past_month_seconds = (
            self.params["interval_past_month"].get("interval_hours", 2) * 3600
        )
        past_month_save_for_seconds = (
            self.params["interval_past_month"].get("save_for_months", 1)
            * 2592000
        )
        past_month_level = self.params["interval_past_month"].get("level", 1)
        past_year_enabled = self.params["interval_past_year"].get(
            "enabled", True
        )
        past_year_seconds = (
            self.params["interval_past_year"].get("interval_days", 1) * 86400
        )
        past_year_save_for_seconds = (
            self.params["interval_past_year"].get("save_for_years", 1)
            * 31536000
        )
        ...
        if past_year_level > past_month_level:
            self.module.fail_json(
                msg="The statistics level for past year can't be higher than past month!"
            )
        if past_month_level > past_week_level:
            self.module.fail_json(
                msg="The statistics level for past month can't be higher than past week!"
            )
        if past_week_level > past_day_level:
            self.module.fail_json(
                msg="The statistics level for past week can't be higher than past day!"
            )

        # Check if state options are valid
        if not past_day_enabled and (
            past_week_enabled or past_month_enabled or past_year_enabled
        ):
            self.module.fail_json(
                msg="The intervals past week, month, and year need to be disabled as well!"
            )
        if not past_week_enabled and (past_month_enabled or past_year_enabled):
            self.module.fail_json(
                msg="The intervals past month, and year need to be disabled as well!"
            )
        if not past_month_enabled and past_year_enabled:
            self.module.fail_json(
                msg="The interval past year need to be disabled as well!"
            )
        if past_year_enabled and (
            not past_day_enabled
            or not past_week_enabled
            or not past_month_enabled
        ):
            self.module.fail_json(
                msg="The intervals past day, week, and month need to be enabled as well!"
            )
        if past_month_enabled and (
            not past_day_enabled or not past_week_enabled
        ):
            self.module.fail_json(
                msg="The intervals past day, and week need to be enabled as well!"
            )
        if past_week_enabled and (not past_day_enabled):
            self.module.fail_json(
                msg="The intervals past day need to be enabled as well!"
            )
        ...
        result["past_month_save_for"] = int(
            past_month_save_for_seconds / 2592000
        )
        ...
        result["past_year_save_for"] = int(
            past_year_save_for_seconds / 31536000
        )
        ...
        for historical_interval in perf_manager.historicalInterval:
            ...
            if historical_interval.name == "Past day" and (
                ...
                if historical_interval.enabled != past_day_enabled:
                    result[
                        "past_day_enabled_previous"
                    ] = historical_interval.enabled
                if historical_interval.samplingPeriod != past_day_seconds:
                    result["past_day_interval_previous"] = int(
                        historical_interval.samplingPeriod / 60
                    )
                if historical_interval.length != past_day_save_for_seconds:
                    result["past_day_save_for_previous"] = int(
                        historical_interval.length / 86400
                    )
                if historical_interval.level != past_day_level:
                    result[
                        "past_day_level_previous"
                    ] = historical_interval.level
            ...
            if historical_interval.name == "Past week" and (
                ...
                if historical_interval.enabled != past_week_enabled:
                    result[
                        "past_week_enabled_previous"
                    ] = historical_interval.enabled
                if historical_interval.samplingPeriod != past_week_seconds:
                    result["past_week_interval_previous"] = int(
                        historical_interval.samplingPeriod / 60
                    )
                if historical_interval.length != past_week_save_for_seconds:
                    result["past_week_save_for_previous"] = int(
                        historical_interval.length / 604800
                    )
                if historical_interval.level != past_week_level:
                    result[
                        "past_week_level_previous"
                    ] = historical_interval.level
            ...
            if historical_interval.name == "Past month" and (
                ...
                if historical_interval.enabled != past_month_enabled:
                    result[
                        "past_month_enabled_previous"
                    ] = historical_interval.enabled
                if historical_interval.samplingPeriod != past_month_seconds:
                    result["past_month_interval_previous"] = int(
                        historical_interval.samplingPeriod / 3600
                    )
                if historical_interval.length != past_month_save_for_seconds:
                    result["past_month_save_for_previous"] = int(
                        historical_interval.length / 2592000
                    )
                if historical_interval.level != past_month_level:
                    result[
                        "past_month_level_previous"
                    ] = historical_interval.level
            ...
            if historical_interval.name == "Past year" and (
                ...
                if historical_interval.enabled != past_year_enabled:
                    result[
                        "past_year_enabled_previous"
                    ] = historical_interval.enabled
                if historical_interval.samplingPeriod != past_year_seconds:
                    result["past_year_interval_previous"] = int(
                        historical_interval.samplingPeriod / 86400
                    )
                if historical_interval.length != past_year_save_for_seconds:
                    result["past_year_save_for_previous"] = int(
                        historical_interval.length / 31536000
                    )
                if historical_interval.level != past_year_level:
                    result[
                        "past_year_level_previous"
                    ] = historical_interval.level
    ...
    def update_perf_interval(self, perf_manager, statistic):
        ...
        try:
            ...
            self.module.fail_json(
                ...
                "the update does not conform to the rules: %s"
