...
class PyVmomiHelper(PyVmomi):
    def __init__(self, module):
        ...
        self.disk_ctl_bus_num_list = dict(
            sata=list(range(0, 4)),
            nvme=list(range(0, 4)),
            scsi=list(range(0, 4)),
        )
    ...
    def gather_disk_controller_facts(self):
        ...
        """
        disk_ctl_facts = dict(
            scsi=dict(), sata=dict(), nvme=dict(), usb2=dict(), usb3=dict()
        )
        for device in self.current_vm_obj.config.hardware.device:
            ctl_facts_dict = dict()
    ...
    def sanitize_disk_controller_config(self):
        ...
        if not self.params.get("controllers"):
            self.module.exit_json(
                changed=False,
                msg="No controller provided for virtual"
        ...
        for ctl_config in controller_config:
            if ctl_config:
                if ctl_config["type"] not in self.usb_device_type.keys():
                    if (
                        ctl_config["state"] == "absent"
                        and ctl_config.get("controller_number") is None
                    ):
                        self.module.fail_json(
                            msg="Disk controller number is required when removing it."
                        )
                    if (
                        ctl_config["state"] == "present"
                        and not exec_get_unused_ctl_bus_number
                    ):
                ...
                if (
                    ctl_config["state"] == "present"
                    and ctl_config["type"] == "nvme"
                ):
                    vm_hwv = int(
                        self.current_vm_obj.config.version.split("-")[1]
                    )
                    if vm_hwv < 13:
                        self.module.fail_json(
                            msg="Can not create new NVMe disk controller due to VM hardware version"
                            " is '%s', not >= 13." % vm_hwv
                        )
        if exec_get_unused_ctl_bus_number:
            for ctl_config in controller_config:
                if (
                    ctl_config
                    and ctl_config["state"] == "present"
                    and ctl_config["type"] not in self.usb_device_type.keys()
                ):
                    if ctl_config["type"] in self.scsi_device_type.keys():
                        if len(self.disk_ctl_bus_num_list["scsi"]) != 0:
                            ctl_config[
                                "controller_number"
                            ] = self.disk_ctl_bus_num_list["scsi"].pop(0)
                        else:
                            ctl_config["controller_number"] = None
                    elif (
                        ctl_config["type"] == "sata"
                        or ctl_config["type"] == "nvme"
                    ):
                        if (
                            len(
                                self.disk_ctl_bus_num_list.get(
                                    ctl_config["type"]
                                )
                            )
                            != 0
    ...
    def configure_disk_controllers(self):
        ...
        for disk_ctl_config in controller_config:
            if disk_ctl_config and disk_ctl_config["state"] == "present":
                ...
                if existing_ctl is not None:
                    ...
                    self.module.warn(
                        "Not find specified controller to remove, type '%s', bus number '%s'."
                        % (
                            disk_ctl_config["type"],
                            disk_ctl_config.get("controller_number"),
                        )
        ...
        try:
            ...
            self.module.fail_json(
                msg="Failed to configure controller on given virtual machine due to invalid"
...
def main():
    ...
    argument_spec.update(
        ...
        controllers=dict(
            ...
            options=dict(
                ...
                controller_number=dict(
                    type="int", choices=[0, 1, 2, 3], required=False
                ),
                type=dict(
                    type="str",
                    choices=[
    ...
    if not vm:
        ...
        module.fail_json(
            msg="Unable to manage disk or USB controllers for non-existing virtual machine '%s'."
            % vm_id
        )
