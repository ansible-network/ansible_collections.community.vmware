...
class VMwareDVSwitchNIOC(PyVmomi):
    ...
    def state_enable_nioc(self):
        ...
        if not self.module.check_mode:
            ...
            self.result["dvswitch_nioc_status"] = (
                "Enabled NIOC with version %s" % self.version
            )
    ...
    def state_update_nioc_version(self):
        ...
        if not self.module.check_mode:
            self.set_nioc_version()
            self.result["dvswitch_nioc_status"] = (
                "Set NIOC to version %s" % self.version
            )
    ...
    def state_update_nioc_resources(self):
        ...
        if not self.module.check_mode:
            self.result[
                "dvswitch_nioc_status"
            ] = "Resource configuration modified"
            self.set_nioc_resources(self.resource_changes)
    ...
    def set_nioc_enabled(self, state):
        try:
            ...
            self.module.fail_json(
                msg="DvsFault while setting NIOC enabled=%r: %s"
                % (state, to_native(dvs_fault.msg))
            )
        except vim.fault.DvsNotAuthorized as auth_fault:
            self.module.fail_json(
                msg="Not authorized to set NIOC enabled=%r: %s"
                % (state, to_native(auth_fault.msg))
            )
        except vmodl.fault.NotSupported as support_fault:
            self.module.fail_json(
                msg="NIOC not supported by DVS: %s"
                % to_native(support_fault.msg)
            )
        except vmodl.RuntimeFault as runtime_fault:
            self.module.fail_json(
                msg="RuntimeFault while setting NIOC enabled=%r: %s"
                % (state, to_native(runtime_fault.msg))
            )
    ...
    def check_nioc_state(self):
        ...
        if self.dvs is None:
            ...
            if (
                self.version
                and self.dvs.config.networkResourceControlVersion
                != self.version
            ):
                return "version"
    ...
    def check_resources(self):
        ...
        if self.dvs is None:
            self.module.fail_json(
                msg="DVS named '%s' was not found" % self.switch
            )
    ...
    def check_resource_state(self, resource):
        ...
        if resource_cfg is None:
            self.module.fail_json(
                msg="NetIOC resource named '%s' was not found"
                % resource["name"]
            )
    ...
    def _update_version3_resources(self, resources):
        ...
        for resource in resources:
            ...
            allocation.allocationInfo = (
                vim.DistributedVirtualSwitch.HostInfrastructureTrafficResource.ResourceAllocation()
            )
            ...
            if "shares_level" in resource:
                ...
                if (
                    ...
                    self.module.fail_json(
                        msg="Resource %s, shares_level set to custom but shares not specified"
                        % resource["name"]
                    )
