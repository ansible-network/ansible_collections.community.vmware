...
class VMwareHostPortGroup(PyVmomi):
    ...
    def __init__(self, module):
        ...
        if self.params["security"]:
            self.sec_promiscuous_mode = self.params["security"].get(
                "promiscuous_mode"
            )
            self.sec_forged_transmits = self.params["security"].get(
                "forged_transmits"
            )
            ...
            for value in ["average_bandwidth", "peak_bandwidth", "burst_size"]:
                if not self.params["traffic_shaping"].get(value):
                    self.module.fail_json(
                        msg="traffic_shaping.%s is a required parameter if traffic_shaping is enabled."
                        % value
                    )
            self.ts_average_bandwidth = self.params["traffic_shaping"].get(
                "average_bandwidth"
            )
            self.ts_peak_bandwidth = self.params["traffic_shaping"].get(
                "peak_bandwidth"
            )
            self.ts_burst_size = self.params["traffic_shaping"].get(
                "burst_size"
            )
        else:
        ...
        if self.params["teaming"]:
            self.teaming_load_balancing = self.params["teaming"].get(
                "load_balancing"
            )
            self.teaming_failure_detection = self.params["teaming"].get(
                "network_failure_detection"
            )
            self.teaming_notify_switches = self.params["teaming"].get(
                "notify_switches"
            )
            self.teaming_failback = self.params["teaming"].get("failback")
            self.teaming_failover_order_active = self.params["teaming"].get(
                "active_adapters"
            )
            self.teaming_failover_order_standby = self.params["teaming"].get(
                "standby_adapters"
            )
            ...
            self.teaming_inbound_policy = self.module.params["teaming"][
                "inbound_policy"
            ]
            self.teaming_rolling_order = self.module.params["teaming"][
                "rolling_order"
            ]
        else:
        ...
        self.hosts = self.get_all_host_objs(
            cluster_name=cluster, esxi_host_name=hosts
        )
        if not self.hosts:
            self.module.fail_json(
    ...
    def process_state(self):
        ...
        for host in self.hosts:
            ...
            if switch_state == "absent":
                self.module.fail_json(
                    msg="The vSwitch '%s' doesn't exist on host '%s'"
                    % (self.switch, host.name)
                )
            ...
            if self.state == "present" and portgroup_state == "present":
                changed, host_results = self.update_host_port_group(
                    host_system=host, portgroup_object=self.portgroup_object
                )
            elif self.state == "present" and portgroup_state == "absent":
                changed, host_results = self.create_host_port_group(
                    host_system=host
                )
            elif self.state == "absent" and portgroup_state == "present":
                changed, host_results = self.remove_host_port_group(
                    host_system=host
                )
            else:
                host_results = dict()
    ...
    def find_portgroup_by_name(
        ...
        for portgroup in portgroups:
            if (
                portgroup.spec.name == portgroup_name
                and portgroup.spec.vswitchName != vswitch_name
            ):
                # portgroup names are unique; there can be only one portgroup with the same name per host
                self.module.fail_json(
                    msg="The portgroup already exists on vSwitch '%s'"
    ...
    def check_if_vswitch_exists(self, host_system):
        ...
        self.switch_object = self.find_vswitch_by_name(
            host_system=host_system, vswitch_name=self.switch
        )
    ...
    def remove_host_port_group(self, host_system):
        ...
        if self.module.check_mode:
            ...
            try:
                host_system.configManager.networkSystem.RemovePortGroup(
                    pgName=self.portgroup
                )
                host_results["msg"] = "Port Group removed"
            except vim.fault.NotFound as not_found:
                self.module.fail_json(
                    msg="Failed to remove Portgroup as it was not found: %s"
                    % to_native(not_found.msg)
                )
            except vim.fault.ResourceInUse as resource_in_use:
                self.module.fail_json(
                    msg="Failed to remove Portgroup as it is in use: %s"
                    % to_native(resource_in_use.msg)
                )
            except vim.fault.HostConfigFault as host_config_fault:
                self.module.fail_json(
                    msg="Failed to remove Portgroup due to configuration failures: %s"
    ...
    def create_host_port_group(self, host_system):
        ...
        if self.module.check_mode:
            ...
            try:
                host_system.configManager.networkSystem.AddPortGroup(
                ...
                self.module.fail_json(
                    msg="Failed to add Portgroup as it already exists: %s"
                    % to_native(already_exists.msg)
                )
            except vim.fault.NotFound as not_found:
                self.module.fail_json(
                    msg="Failed to add Portgroup as vSwitch was not found: %s"
                    % to_native(not_found.msg)
                )
            except vim.fault.HostConfigFault as host_config_fault:
                self.module.fail_json(
        ...
        host_results["traffic_shaping"] = (
            "No override" if self.ts_enabled is None else self.ts_enabled
        )
        host_results["load_balancing"] = (
            "No override"
            if self.teaming_load_balancing is None
            else self.teaming_load_balancing
        )
        host_results["notify_switches"] = (
            "No override"
            if self.teaming_notify_switches is None
            else self.teaming_notify_switches
        )
        host_results["failback"] = (
            "No override"
            if self.teaming_failback is None
            else self.teaming_failback
        )
        host_results["failover_active"] = (
            "No override"
            if self.teaming_failover_order_active is None
            else self.teaming_failover_order_active
        )
        host_results["failover_standby"] = (
            "No override"
            if self.teaming_failover_order_standby is None
            else self.teaming_failover_order_standby
        )
        host_results["failure_detection"] = (
            "No override"
    ...
    def update_host_port_group(self, host_system, portgroup_object):
        ...
        if spec.policy.security:
            ...
            if changed_security:
                if self.sec_promiscuous_mode is None:
                    host_results[
                        "sec_promiscuous_mode_previous"
                    ] = "No override"
                else:
                    host_results[
                        "sec_promiscuous_mode_previous"
                    ] = promiscuous_mode_previous
                if self.sec_mac_changes is None:
                ...
                if self.sec_forged_transmits is None:
                    host_results[
                        "sec_forged_transmits_previous"
                    ] = "No override"
                else:
                    host_results[
                        "sec_forged_transmits_previous"
                    ] = forged_transmits_previous
        else:
        ...
        if self.ts_enabled is None:
            ...
            if self.ts_enabled:
                ...
                host_results[
                    "traffic_shaping_avg_bandw"
                ] = ts_average_bandwidth
        ...
        if (
            ...
            if spec.policy.shapingPolicy.enabled:
                if self.ts_enabled:
                    if (
                        spec.policy.shapingPolicy.averageBandwidth
                        != ts_average_bandwidth
                    ):
                        changed = True
                        changed_list.append("Average bandwidth")
                        host_results[
                            "traffic_shaping_avg_bandw_previous"
                        ] = spec.policy.shapingPolicy.averageBandwidth
                        spec.policy.shapingPolicy.averageBandwidth = (
                            ts_average_bandwidth
                        )
                    if (
                        spec.policy.shapingPolicy.peakBandwidth
                        != ts_peak_bandwidth
                    ):
                        changed = True
                        changed_list.append("Peak bandwidth")
                        host_results[
                            "traffic_shaping_peak_bandw_previous"
                        ] = spec.policy.shapingPolicy.peakBandwidth
                        spec.policy.shapingPolicy.peakBandwidth = (
                            ts_peak_bandwidth
                        )
                    if spec.policy.shapingPolicy.burstSize != ts_burst_size:
                        ...
                        ] = spec.policy.shapingPolicy.burstSize
        ...
        if spec.policy.nicTeaming:
            ...
            if spec.policy.nicTeaming.policy:
                if (
                    spec.policy.nicTeaming.policy
                    != self.teaming_load_balancing
                ):
                    changed = True
                    changed_list.append("Load balancing")
                    host_results[
                        "load_balancing_previous"
                    ] = spec.policy.nicTeaming.policy
            ...
            if spec.policy.nicTeaming.notifySwitches is not None:
                if self.teaming_notify_switches is not None:
                    if (
                        ...
                        is not self.teaming_notify_switches
                    ):
                        changed = True
            ...
            if spec.policy.nicTeaming.rollingOrder is not None:
                if self.teaming_failback is not None:
                    ...
                    if (
                        spec.policy.nicTeaming.rollingOrder
                        is self.teaming_failback
                    ):
                        changed = True
            ...
            if (
                self.teaming_failover_order_active is None
                and self.teaming_failover_order_standby is None
            ):
            ...
            if spec.policy.nicTeaming.nicOrder:
                if (
                    self.teaming_failover_order_active
                    or self.teaming_failover_order_standby
                ):
                    if (
                ...
                if (
                    ...
                    spec.policy.nicTeaming.nicOrder = (
                        self.create_nic_order_policy()
                    )
            ...
            if (
                ...
                if self.teaming_failure_detection == "link_status_only":
                    if (
                        spec.policy.nicTeaming.failureCriteria.checkBeacon
                        is True
                    ):
                        changed = True
                        changed_list.append("Network failure detection")
                        host_results[
                            "failure_detection_previous"
                        ] = "beacon_probing"
                        spec.policy.nicTeaming.failureCriteria.checkBeacon = (
                            False
                        )
                ...
                if self.teaming_failure_detection:
                    spec.policy.nicTeaming.failureCriteria = (
                        self.create_nic_failure_policy()
                    )
                    changed = True
                    changed_list.append("Network failure detection")
            ...
            if not self.module.check_mode:
                try:
                    host_system.configManager.networkSystem.UpdatePortGroup(
                        pgName=self.portgroup, portgrp=spec
                    )
                except vim.fault.AlreadyExists as already_exists:
                    self.module.fail_json(
    ...
    def create_teaming_policy(self):
        ...
        if not all(
            ...
            if self.teaming_failure_detection is None:
                ...
                teaming_policy.failureCriteria = (
                    self.create_nic_failure_policy()
                )
    ...
    def create_nic_order_policy(self):
        ...
        for active_nic in self.teaming_failover_order_active:
            if active_nic not in self.switch_object.spec.bridge.nicDevice:
                self.module.fail_json(
                    msg="NIC '%s' (active) is not configured on vSwitch '%s'"
                    % (active_nic, self.switch)
                )
        for standby_nic in self.teaming_failover_order_standby:
            if standby_nic not in self.switch_object.spec.bridge.nicDevice:
                self.module.fail_json(
                    msg="NIC '%s' (standby) is not configured on vSwitch '%s'"
                    % (standby_nic, self.switch)
                )
...
def main():
    ...
    argument_spec.update(
        dict(
            portgroup=dict(
                type="str", required=True, aliases=["portgroup_name"]
            ),
            switch=dict(
                type="str", required=True, aliases=["switch_name", "vswitch"]
            ),
            vlan_id=dict(
                type="int", required=False, default=0, aliases=["vlan"]
            ),
    ...
    module = AnsibleModule(
        ...
        required_one_of=[["cluster_name", "hosts"]],
