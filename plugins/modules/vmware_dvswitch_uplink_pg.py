...
class VMwareDvSwitchUplinkPortgroup(PyVmomi):
    ...
    def __init__(self, module):
        ...
        if self.dvs is None:
            self.module.fail_json(
                msg="Failed to find DVS %s" % self.switch_name
            )
    ...
    def ensure(self):
        ...
        changed = (
            changed_uplink_pg_policy
        ) = changed_vlan_trunk_range = changed_lacp = False
        ...
        uplink_pg_policy_spec = (
            vim.dvs.VmwareDistributedVirtualSwitch.VMwarePortgroupPolicy()
        )
        uplink_pg_policy_spec.portConfigResetAtDisconnect = (
            self.uplink_pg_reset
        )
        uplink_pg_policy_spec.blockOverrideAllowed = self.uplink_pg_block_ports
        uplink_pg_policy_spec.vendorConfigOverrideAllowed = (
            self.uplink_pg_vendor_conf
        )
        ...
        # Check policies
        if (
            uplink_pg_config.policy.portConfigResetAtDisconnect
            != self.uplink_pg_reset
        ):
            changed_uplink_pg_policy = True
            results[
                "adv_reset_at_disconnect_previous"
            ] = uplink_pg_config.policy.portConfigResetAtDisconnect
        if (
            uplink_pg_config.policy.blockOverrideAllowed
            != self.uplink_pg_block_ports
        ):
            changed_uplink_pg_policy = True
            results[
                "adv_block_ports_previous"
            ] = uplink_pg_config.policy.blockOverrideAllowed
        if (
            uplink_pg_config.policy.vendorConfigOverrideAllowed
            != self.uplink_pg_vendor_conf
        ):
            changed_uplink_pg_policy = True
            results[
                "adv_vendor_conf_previous"
            ] = uplink_pg_config.policy.vendorConfigOverrideAllowed
        if uplink_pg_config.policy.vlanOverrideAllowed != self.uplink_pg_vlan:
            changed_uplink_pg_policy = True
            results[
                "adv_vlan_previous"
            ] = uplink_pg_config.policy.vlanOverrideAllowed
        if (
            uplink_pg_config.policy.ipfixOverrideAllowed
            != self.uplink_pg_netflow
        ):
            changed_uplink_pg_policy = True
            results[
                "adv_netflow_previous"
            ] = uplink_pg_config.policy.ipfixOverrideAllowed
        if (
            uplink_pg_config.policy.trafficFilterOverrideAllowed
            != self.uplink_pg_tf
        ):
            changed_uplink_pg_policy = True
            results[
                "adv_traffic_filtering_previous"
            ] = uplink_pg_config.policy.trafficFilterOverrideAllowed
        ...
        uplink_pg_spec.defaultPortConfig = (
        ...
        for vlan_id_range in vlan_id_ranges:
            ...
            ) in uplink_pg_config.defaultPortConfig.vlan.vlanId:
            ...
            vlan_id_list.append(
                vim.NumericRange(
                    start=int(vlan_id_start), end=int(vlan_id_end)
                )
            )
        ...
        if lacp_support_mode == "basic":
            ...
            if self.lacp_status == "enabled":
                lacp_enabled = True
            if (
                uplink_pg_config.defaultPortConfig.lacpPolicy.enable.value
                != lacp_enabled
            ):
                changed_lacp = True
            ...
            if (
                lacp_enabled
                and uplink_pg_config.defaultPortConfig.lacpPolicy.mode.value
                != self.lacp_mode
            ):
                ...
                results[
                    "lacp_mode_previous"
                ] = uplink_pg_config.defaultPortConfig.lacpPolicy.mode.value
        ...
        if (
            uplink_pg_config.defaultPortConfig.ipfixEnabled.value
            != self.uplink_pg_netflow_enabled
        ):
            changed = True
            results[
                "netflow_enabled_previous"
            ] = uplink_pg_config.defaultPortConfig.ipfixEnabled.value
            changed_list.append("netflow")
            uplink_pg_spec.defaultPortConfig.ipfixEnabled = (
                netflow_enabled_spec
            )
        ...
        if (
            uplink_pg_config.defaultPortConfig.blocked.value
            != self.uplink_pg_block_all_ports
        ):
            changed = True
            changed_list.append("block all ports")
            results[
                "block_all_ports_previous"
            ] = uplink_pg_config.defaultPortConfig.blocked.value
            ...
            if not self.module.check_mode:
                try:
                    task = self.dvs.config.uplinkPortgroup[
                        0
                    ].ReconfigureDVPortgroup_Task(uplink_pg_spec)
                    wait_for_task(task)
    ...
    argument_spec.update(
        dict(
            ...
            lacp=dict(
                ...
                default=dict(status="disabled", mode="passive"),
            ),
