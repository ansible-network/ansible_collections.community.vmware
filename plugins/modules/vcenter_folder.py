...
class VmwareFolderManager(PyVmomi):
        ...
        if state == "present":
            ...
            if parent_folder:
                ...
                if folder_obj:
                    ...
                    results["result"]["msg"] = (
                        "Folder %s already exists" % folder_name
                    )
                    self.module.exit_json(**results)
            ...
            try:
                if parent_folder and p_folder_obj:
                    if self.module.check_mode:
                        results["msg"] = (
                            "Folder '%s' of type '%s' under '%s' will be created."
                        ...
                        results["result"]["msg"] = (
                            "Folder '%s' of type '%s' under '%s' created"
                            " successfully."
                            % (folder_name, folder_type, parent_folder)
                        )
                    ...
                    if self.module.check_mode:
                        results["msg"] = (
                            "Folder '%s' of type '%s' will be created."
                        ...
                        results["result"]["msg"] = (
                            "Folder '%s' of type '%s' created successfully."
                            % (folder_name, folder_type)
                        )
                        results["result"]["path"] = self.get_folder_path(
                            new_folder
                ...
                results["msg"] = (
                    "Failed to create folder as another object has same name"
                    " in the same target folder : %s"
                    % to_native(duplicate_name.msg)
                )
            ...
            if parent_folder:
                ...
                if not p_folder_obj:
                    self.module.fail_json(
                        msg="Parent folder %s does not exist" % parent_folder
                    )
                ...
                try:
                    if parent_folder:
                        if self.module.check_mode:
                            results["changed"] = True
                            results["msg"] = (
                                "Folder '%s' of type '%s' under '%s' will be removed."
                                % (folder_name, folder_type, parent_folder)
                            )
                        else:
                            if folder_type == "vm":
                        ...
                        if self.module.check_mode:
                            results["changed"] = True
                            results["msg"] = (
                                "Folder '%s' of type '%s' will be removed."
                                % (folder_name, folder_type)
                            )
                        else:
                            if folder_type == "vm":
    ...
    def get_folder(
        self, datacenter_name, folder_name, folder_type, parent_folder=None
    ):
        """
        ...
        for folder in folder_objs:
            if parent_folder:
                if (
                    ...
                    and self.datacenter_folder_type[folder_type].childType
                    == folder.childType
                ):
                    return folder
            else:
                if (
                    ...
                    and self.datacenter_folder_type[folder_type].childType
                    == folder.childType
                    and folder.parent.parent.name == datacenter_name
                ):  # e.g. folder.parent.parent.name == /DC01/host/folder
                    return folder
...
def main():
    ...
    if len(module.params.get("folder_name")) > 79:
        module.fail_json(
            msg="Failed to manage folder as folder_name can only contain 80 characters."
        )
