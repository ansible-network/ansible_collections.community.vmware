...
class VMwareDvSwitchLacp(PyVmomi):
    ...
    def ensure(self):
        ...
        if self.dvs.config.lacpApiVersion != lacp_support_mode:
            ...
            results["support_mode_previous"] = self.get_lacp_support_mode(
                self.dvs.config.lacpApiVersion
            )
        ...
        if self.link_aggregation_groups and not (
            ...
            if self.dvs.config.lacpGroupConfig:
                ...
                for lag in self.link_aggregation_groups:
                    lag_name, lag_mode, lag_uplink_number, lag_load_balancing_mode = self.get_lacp_lag_options(
                        lag
                    )
                    lag_found = False
        ...
        if changed:
            ...
            if not self.module.check_mode:
                if (
                    changed_support_mode
                    and self.support_mode == "basic"
                    and changed_lags
                ):
    ...
    def get_lacp_lag_options(self, lag):
        ...
        if lag_name is None:
            self.module.fail_json(
                msg="Please specify name in lag options as it's a required parameter"
            )
        lag_mode = lag.get("mode", None)
        if lag_mode is None:
            self.module.fail_json(
                msg="Please specify mode in lag options as it's a required parameter"
            )
        lag_uplink_number = lag.get("uplink_number", None)
        if lag_uplink_number is None:
            self.module.fail_json(
                msg="Please specify uplink_number in lag options as it's a required parameter"
            )
        elif lag_uplink_number > 30:
            self.module.fail_json(
                msg="More than 30 uplinks are not supported in a single LAG!"
            )
        ...
        if lag_load_balancing_mode is None:
            self.module.fail_json(
                msg="Please specify load_balancing_mode in lag options as it's a required parameter"
            )
        elif lag_load_balancing_mode not in supported_lb_modes:
            self.module.fail_json(
                msg="The specified load balancing mode '%s' isn't supported!"
                % lag_load_balancing_mode
            )
        return lag_name, lag_mode, lag_uplink_number, lag_load_balancing_mode

    @staticmethod
    def create_lacp_group_spec(
        operation, key, name, uplink_number, mode, load_balancing_mode
    ):
        """
        ...
        if not operation == "remove":
            ...
            lacp_spec.lacpGroupConfig.loadbalanceAlgorithm = (
                load_balancing_mode
            )
    ...
    def update_lacp_group_config(self, switch_object, lacp_group_spec):
        ...
        try:
            task = switch_object.UpdateDVSLacpGroupConfig_Task(
                lacpGroupSpec=lacp_group_spec
            )
            result = wait_for_task(task)
            ...
            self.module.fail_json(
                msg="Failed to update Link Aggregation Group : %s"
