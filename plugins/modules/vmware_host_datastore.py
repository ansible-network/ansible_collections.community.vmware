...
class VMwareHostDatastore(PyVmomi):
    def __init__(self, module):
        ...
        if self.is_vcenter():
            ...
            if self.esxi is None:
                self.module.fail_json(
                    msg="Failed to find ESXi hostname %s" % self.esxi_hostname
                )
        else:
    ...
    def get_used_disks_names(self):
        ...
        for (
            each_vol_mount_info
        ) in storage_system.fileSystemVolumeInfo.mountInfo:
    ...
    def umount_datastore_host(self):
        ...
        if not ds:
            self.module.fail_json(
                msg="No datastore found with name %s" % self.datastore_name
            )
        if self.module.check_mode is False:
            error_message_umount = (
                "Cannot umount datastore %s from host %s"
                % (self.datastore_name, self.esxi.name)
            )
            try:
                ...
                self.module.fail_json(
                    msg="%s: %s" % (error_message_umount, to_native(fault.msg))
                )
            except Exception as e:
                self.module.fail_json(
                    msg="%s: %s" % (error_message_umount, to_native(e))
                )
        self.module.exit_json(
            changed=True,
            result="Datastore %s on host %s"
            % (self.datastore_name, self.esxi.name),
        )
    ...
    def mount_nfs_datastore_host(self):
        if self.module.check_mode is False:
            ...
            error_message_mount = "Cannot mount datastore %s on host %s" % (
                self.datastore_name,
                self.esxi.name,
            )
            try:
                ds = self.esxi.configManager.datastoreSystem.CreateNasDatastore(
                    mnt_specs
                )
                if not ds:
                ...
                self.module.fail_json(
                    msg="%s: %s" % (error_message_mount, to_native(fault.msg))
                )
            except Exception as e:
                self.module.fail_json(
                    msg="%s : %s" % (error_message_mount, to_native(e))
                )
        self.module.exit_json(
            changed=True,
            result="Datastore %s on host %s"
    ...
    def mount_vmfs_datastore_host(self):
        if self.module.check_mode is False:
            ...
            if self.vmfs_device_name in self.get_used_disks_names():
                error_message_used_disk = (
                    "VMFS disk %s already in use" % self.vmfs_device_name
                )
            ...
            try:
                vmfs_ds_options = ds_system.QueryVmfsDatastoreCreateOptions(
                    host_ds_system, ds_path, self.vmfs_version
                )
                vmfs_ds_options[0].spec.vmfs.volumeName = self.datastore_name
                ds = ds_system.CreateVmfsDatastore(
                    host_ds_system, vmfs_ds_options[0].spec
                )
            except (
                vim.fault.NotFound,
                vim.fault.DuplicateName,
...
def main():
    ...
    argument_spec.update(
        datacenter_name=dict(
            type="str", required=False, removed_in_version=2.11
        ),
    ...
    if module.params["state"] == "present":
        if (
            module.params["datastore_type"] == "nfs"
            and not module.params["nfs_server"]
        ):
            msg = "Missing nfs_server with datastore_type = nfs"
            module.fail_json(msg=msg)

        if (
            module.params["datastore_type"] == "nfs41"
            and not module.params["nfs_server"]
        ):
        ...
        if (
            module.params["datastore_type"] == "vmfs"
            and not module.params["vmfs_device_name"]
        ):
