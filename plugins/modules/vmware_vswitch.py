...
class VMwareHostVirtualSwitch(PyVmomi):
    def __init__(self, module):
        ...
        if hosts:
            ...
            self.module.fail_json(
                msg="Failed to get details of ESXi server."
        ...
        if self.params.get("state") == "present":
            ...
            available_pnic = [
                pnic.device for pnic in network_manager.networkInfo.pnic
            ]
            ...
            for desired_pnic in self.nics:
                if desired_pnic not in available_pnic:
                    ...
                    self.module.fail_json(
                        msg="Specified Physical NIC '%s' does not"
                        " exists on given ESXi '%s'."
                        % (desired_pnic, self.host_system.name)
                    )
                for vswitch in self.available_vswitches:
                    if (
                        desired_pnic
                        in self.available_vswitches[vswitch]["pnic"]
                        and vswitch != self.switch
                    ):
                        # Check if pnic is already part of some other vSwitch
                        self.module.fail_json(
                            msg="Specified Physical NIC '%s' is already used"
        ...
        try:
            ...
            if network_mgr:
                network_mgr.AddVirtualSwitch(
                    vswitchName=self.switch, spec=vss_spec
                )
                results["changed"] = True
                results["result"] = (
                    "vSwitch '%s' is created successfully" % self.switch
                )
            else:
                self.module.fail_json(
                    msg="Failed to find network manager for ESXi system"
                )
            ...
            self.module.fail_json(
                msg="Failed to add vSwitch '%s' as physical network adapter"
                " being bridged is already in use: %s"
                % (self.switch, to_native(resource_used.msg))
            )
        except vim.fault.HostConfigFault as host_config_fault:
            self.module.fail_json(
                msg="Failed to add vSwitch '%s' due to host"
                " configuration fault : %s"
                % (self.switch, to_native(host_config_fault.msg))
            )
        except vmodl.fault.InvalidArgument as invalid_argument:
            self.module.fail_json(
                msg="Failed to add vSwitch '%s', this can be due to either of following :"
            ...
            self.module.fail_json(
                msg="Failed to add vSwitch '%s' due to : %s"
                % (self.switch, to_native(system_error.msg))
            )
        except Exception as generic_exc:
            self.module.fail_json(
                msg="Failed to add vSwitch '%s' due to"
                " generic exception : %s"
                % (self.switch, to_native(generic_exc))
            )
    ...
    def state_destroy_vswitch(self):
        ...
        try:
            self.host_system.configManager.networkSystem.RemoveVirtualSwitch(
                self.vss.name
            )
            results["changed"] = True
            results["result"] = (
                "vSwitch '%s' removed successfully." % self.vss.name
            )
        except vim.fault.NotFound as vswitch_not_found:
            results["result"] = "vSwitch '%s' not available. %s" % (
                self.switch,
                to_native(vswitch_not_found.msg),
            )
        except vim.fault.ResourceInUse as vswitch_in_use:
            self.module.fail_json(
                msg="Failed to remove vSwitch '%s' as vSwitch"
            ...
            self.module.fail_json(
                msg="Failed to remove vSwitch '%s' due to host"
                " configuration fault : %s"
                % (self.switch, to_native(host_config_fault.msg))
            )
        except Exception as generic_exc:
            self.module.fail_json(
                msg="Failed to remove vSwitch '%s' due to generic"
                " exception : %s" % (self.switch, to_native(generic_exc))
            )
    ...
    def state_update_vswitch(self):
        ...
        """
        results = dict(
            changed=False, result="No change in vSwitch '%s'" % self.switch
        )
        ...
        if (
            vswitch_pnic_info["mtu"] != self.mtu
            or vswitch_pnic_info["num_ports"] != self.number_of_ports
        ):
            diff = True
        ...
        try:
            if diff:
                ...
                network_mgr = self.host_system.configManager.networkSystem
                if network_mgr:
                    network_mgr.UpdateVirtualSwitch(
                        vswitchName=self.switch, spec=vss_spec
                    )
                    results["changed"] = True
                    results["result"] = (
                        "vSwitch '%s' is updated successfully" % self.switch
                    )
                else:
                    self.module.fail_json(
                        msg="Failed to find network manager for ESXi system."
            ...
            self.module.fail_json(
                msg="Failed to update vSwitch '%s' as physical network adapter"
                " being bridged is already in use: %s"
                % (self.switch, to_native(resource_used.msg))
            )
        except vim.fault.NotFound as not_found:
            self.module.fail_json(
                msg="Failed to update vSwitch with name '%s'"
                " as it does not exists: %s"
                % (self.switch, to_native(not_found.msg))
            )

        except vim.fault.HostConfigFault as host_config_fault:
            self.module.fail_json(
                msg="Failed to update vSwitch '%s' due to host"
                " configuration fault : %s"
                % (self.switch, to_native(host_config_fault.msg))
            )
        except vmodl.fault.InvalidArgument as invalid_argument:
            self.module.fail_json(
                msg="Failed to update vSwitch '%s', this can be due to either of following :"
                " 1. vSwitch Name exceeds the maximum allowed length,"
                " 2. Number of ports specified falls out of valid range,"
                " 3. Network policy is invalid,"
                " 4. Beacon configuration is invalid : %s"
            ...
            self.module.fail_json(
                msg="Failed to update vSwitch '%s' due to : %s"
                % (self.switch, to_native(system_error.msg))
            )
        except vmodl.fault.NotSupported as not_supported:
            self.module.fail_json(
                msg="Failed to update vSwitch '%s' as network adapter teaming policy"
                " is set but is not supported : %s"
                % (self.switch, to_native(not_supported.msg))
            )
        except Exception as generic_exc:
            self.module.fail_json(
                msg="Failed to update vSwitch '%s' due to"
                " generic exception : %s"
                % (self.switch, to_native(generic_exc))
            )
        self.module.exit_json(**results)
