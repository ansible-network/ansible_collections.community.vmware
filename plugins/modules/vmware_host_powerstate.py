    ...
    def ensure(self):
        ...
        for host in self.hosts:
            ...
            if not host.runtime.inMaintenanceMode and not force:
                self.module.fail_json(
                    msg="Current host system '%s' is not in maintenance mode,"
                    " please specify 'force' as True to proceed." % host.name
                )
            if host.runtime.connectionState == "notResponding":
                self.module.fail_json(
                    msg="Current host system '%s' can not be set in '%s'"
            ...
            if state == "reboot-host" and not host.capability.rebootSupported:
                self.module.fail_json(
                    msg="Current host '%s' can not be rebooted as the host system"
                    " does not have capability to reboot." % host.name
                )
            elif (
                state == "shutdown-host"
                and not host.capability.shutdownSupported
            ):
                self.module.fail_json(
                    msg="Current host '%s' can not be shut down as the host system"
                    " does not have capability to shut down." % host.name
                )
            elif (
                state in ["power-down-to-standby", "power-up-from-standby"]
                and not host.capability.standbySupported
            ):
                self.module.fail_json(
                    msg="Current host '%s' can not be '%s' as the host system"
            ...
            if not self.module.check_mode:
                try:
                    ...
                    self.module.fail_json(
                        msg="Failed to %s as host system due to : %s"
                        % (verb, str(task_error))
                    )
                except Exception as generic_exc:
                    self.module.fail_json(
                        msg="Failed to %s due to generic exception : %s"
                        % (host.name, to_native(generic_exc))
                    )
            else:
                # Check mode
                changed = True
