...
def create(module):
    ...
    if network_name is not None:
        vm_ip = connect_to_network(
            module, vdc_name, vapp_name, network_name, network_mode
        )
        return vm_ip
...
def connect_to_network(
    ...
    nets = filter(
        lambda n: n.name == network_name, module.vca.get_networks(vdc_name)
    )
    if len(nets) != 1:
    ...
    task = the_vapp.connect_vms(
        nets[0].name, connection_index=0, ip_allocation_mode=ip_allocation_mode
    )
    if result is None:
        module.fail_json(msg="Failed to complete task")
...
def get_vm_details(module):
    ...
    for vm in the_vapp.me.Children.Vm:
        ...
        if customization_section.get_AdminPasswordEnabled():
            the_vm_details[
                "vm_admin_password"
            ] = customization_section.get_AdminPassword()

        virtual_hardware_section = filter(
            lambda section: section.__class__.__name__
            ...
            if item.Connection:
                for c in item.Connection:
                    if c.anyAttributes_.get(_url):
                        ips.append(c.anyAttributes_.get(_url))
    if len(ips) > 0:
...
def main():
    argument_spec = dict(
        ...
        vm_memory=dict(),
        operation=dict(
            default=DEFAULT_VAPP_OPERATION, choices=VAPP_OPERATIONS
        ),
