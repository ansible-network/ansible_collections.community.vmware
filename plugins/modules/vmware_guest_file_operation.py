...
class VmwareGuestFileManager(PyVmomi):
    ...
    def directory(self):
        ...
        if operation in ("create", "mktemp"):
            try:
                if operation == "create":
                    ...
                    newdir = file_manager.CreateTemporaryDirectoryInGuest(
                        vm=self.vm, auth=creds, prefix=prefix, suffix=suffix
                    )
                    result["dir"] = newdir
            except vim.fault.FileAlreadyExists as file_already_exists:
                result["changed"] = False
                result["msg"] = "Guest directory %s already exist: %s" % (
                    path,
                    to_native(file_already_exists.msg),
                )
            except vim.fault.GuestPermissionDenied as permission_denied:
                self.module.fail_json(
                    msg="Permission denied for path %s : %s"
                    % (path, to_native(permission_denied.msg)),
                    uuid=self.vm.summary.config.uuid,
                )
            except vim.fault.InvalidGuestLogin as invalid_guest_login:
                self.module.fail_json(
                    msg="Invalid guest login for user %s : %s"
                    % (vm_username, to_native(invalid_guest_login.msg)),
                    uuid=self.vm.summary.config.uuid,
                )
            # other exceptions
            except Exception as e:
                self.module.fail_json(
                    msg="Failed to Create directory into VM VMware exception : %s"
                    % to_native(e),
                    uuid=self.vm.summary.config.uuid,
                )
        ...
        if operation == "delete":
            try:
                ...
                result["changed"] = False
                result["msg"] = "Guest directory %s not exists %s" % (
                    path,
                    to_native(file_not_found.msg),
                )
            except vim.fault.FileFault as e:
                self.module.fail_json(
                    msg="FileFault : %s" % e.msg,
                    uuid=self.vm.summary.config.uuid,
                )
            except vim.fault.GuestPermissionDenied as permission_denied:
                self.module.fail_json(
            ...
            # other exceptions
                ...
                self.module.fail_json(
                    msg="Failed to Delete directory into Vm VMware exception : %s"
                    % to_native(e),
                    uuid=self.vm.summary.config.uuid,
                )
    ...
    def fetch(self):
        ...
        try:
            fileTransferInfo = file_manager.InitiateFileTransferFromGuest(
                vm=self.vm, auth=creds, guestFilePath=src
            )
            ...
            try:
                ...
                self.module.fail_json(
                    msg="local file write exception : %s" % to_native(e),
                    uuid=self.vm.summary.config.uuid,
                )
        except vim.fault.FileNotFound as file_not_found:
            self.module.fail_json(
                msg="Guest file %s does not exist : %s"
                % (src, to_native(file_not_found.msg)),
                uuid=self.vm.summary.config.uuid,
            )
        except vim.fault.FileFault as e:
            self.module.fail_json(
                msg="FileFault : %s" % to_native(e.msg),
                uuid=self.vm.summary.config.uuid,
            )
        except vim.fault.GuestPermissionDenied:
            self.module.fail_json(
                msg="Permission denied to fetch file %s" % src,
                uuid=self.vm.summary.config.uuid,
            )
        except vim.fault.InvalidGuestLogin:
            self.module.fail_json(
                msg="Invalid guest login for user %s" % vm_username,
                uuid=self.vm.summary.config.uuid,
            )
        # other exceptions
        except Exception as e:
            self.module.fail_json(
                msg="Failed to Fetch file from Vm VMware exception : %s"
                % to_native(e),
                uuid=self.vm.summary.config.uuid,
            )
    ...
    def copy(self):
        ...
        if os.path.isdir(b_src):
            self.module.fail_json(
                msg="copy does not support copy of directory: %s" % src
            )
        ...
        try:
            ...
            url = url.replace("*", hostname)
            resp, info = urls.fetch_url(
                self.module, url, data=data, method="PUT"
            )

            status_code = info["status"]
            if status_code != 200:
                self.module.fail_json(
                    msg="problem during file transfer, http message:%s" % info,
                    uuid=self.vm.summary.config.uuid,
                )
            ...
            self.module.fail_json(
                msg="Permission denied to copy file into "
                "destination %s : %s"
                % (dest, to_native(permission_denied.msg)),
                uuid=self.vm.summary.config.uuid,
            )
        except vim.fault.InvalidGuestLogin as invalid_guest_login:
            self.module.fail_json(
                msg="Invalid guest login for user"
                " %s : %s" % (vm_username, to_native(invalid_guest_login.msg))
            )
        # other exceptions
        except Exception as e:
            self.module.fail_json(
                msg="Failed to Copy file to Vm VMware exception : %s"
                % to_native(e),
                uuid=self.vm.summary.config.uuid,
            )
        return result
...
def main():
    ...
    if module.params["directory"]:
        if (
            module.params["directory"]["operation"] in ("create", "delete")
            and not module.params["directory"]["path"]
        ):
            module.fail_json(
                msg='directory.path is required when operation is "create" or "delete"'
            )
        if module.params["directory"]["operation"] == "mktemp" and not (
            ...
            module.fail_json(
                msg='directory.prefix and directory.suffix are required when operation is "mktemp"'
            )

    if (
        module.params["vm_id_type"] == "inventory_path"
        and not module.params["folder"]
    ):
        module.fail_json(
