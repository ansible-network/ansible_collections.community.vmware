...
class CrossVCCloneManager(PyVmomi):
    def __init__(self, module):
        ...
        self.destination_vcenter_username = self.params[
            "destination_vcenter_username"
        ]
        self.destination_vcenter_password = self.params[
            "destination_vcenter_password"
        ]
        self.destination_vcenter_port = self.params.get("port", 443)
        self.destination_vcenter_validate_certs = self.params.get(
            "destination_vcenter_validate_certs", None
        )
    ...
    def get_new_vm_info(self, vm):
        ...
        if vm_obj is None:
            self.module.fail_json(
                msg="Newly cloned VM is not found in the destination VCenter"
            )
        else:
    ...
    def clone(self):
        ...
        task = self.vm_obj.Clone(
            folder=vm_folder, name=vm_name, spec=self.clone_spec
        )
        wait_for_task(task)
    ...
    def sanitize_params(self):
        ...
        datastore_cluster = find_obj(
            self.destination_content, [vim.StoragePod], datastore_name
        )
    ...
    def populate_specs(self):
        ...
        self.service_locator.instanceUuid = (
            self.destination_content.about.instanceUuid
        )
        self.service_locator.url = (
            "https://"
        ...
        self.clone_spec.powerOn = (
            True if self.params["state"].lower() == "poweredon" else False
        )
...
def main():
    ...
    argument_spec.update(
        ...
        destination_vcenter_password=dict(
        ...
        state=dict(
            type="str", default="present", choices=["present", "poweredon"]
        ),
    )
    ...
    module = AnsibleModule(
        ...
        required_one_of=[["uuid", "name", "moid"]],
        mutually_exclusive=[["uuid", "name", "moid"]],
    )
