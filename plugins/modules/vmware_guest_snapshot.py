...
class PyVmomiHelper(PyVmomi):
    ...
    def snapshot_vm(self, vm):
        ...
        if vm.snapshot is not None:
            ...
            if snap_obj:
                ...
                self.module.exit_json(
                    changed=False,
                    msg="Snapshot named [%(snapshot_name)s] already exists and is current."
                    % self.module.params,
                )
        ...
        try:
            ...
            self.module.fail_json(
                msg="Failed to take snapshot due to VMware Licence"
                " restriction : %s" % to_native(exc.msg)
            )
        except Exception as exc:
            self.module.fail_json(
                msg="Failed to create snapshot of virtual machine"
    ...
    def rename_snapshot(self, vm):
        if vm.snapshot is None:
            vm_id = (
                ...
                or self.module.params.get("name")
                or self.params.get("moid")
            )
            self.module.fail_json(
                msg="virtual machine - %s doesn't have any snapshots" % vm_id
            )

        snap_obj = self.get_snapshots_by_name_recursively(
            vm.snapshot.rootSnapshotList, self.module.params["snapshot_name"]
        )
        task = None
        if len(snap_obj) == 1:
            snap_obj = snap_obj[0].snapshot
            if (
                self.module.params["new_snapshot_name"]
                and self.module.params["new_description"]
            ):
                task = snap_obj.RenameSnapshot(
                    name=self.module.params["new_snapshot_name"],
    ...
    def remove_or_revert_snapshot(self, vm):
        if vm.snapshot is None:
            vm_name = self.module.params.get("uuid") or self.module.params.get(
                "name"
            )
            if self.module.params.get("state") == "revert":
                self.module.fail_json(
                    msg="virtual machine - %s does not"
                    " have any snapshots to revert to." % vm_name
                )
            self.module.exit_json(
                msg="virtual machine - %s doesn't have any"
        ...
        if len(snap_obj) == 1:
            ...
            if self.module.params["state"] == "absent":
                ...
                remove_children = self.module.params.get(
            ...
            self.module.exit_json(
                msg="Couldn't find any snapshots with"
                " specified name: %s on VM: %s"
                % (self.module.params["snapshot_name"], vm_id)
            )
    ...
    def apply_snapshot_op(self, vm):
        ...
        if self.module.params["state"] == "present":
            if (
                self.module.params["new_snapshot_name"]
                or self.module.params["new_description"]
            ):
...
def main():
    ...
    if not vm:
        ...
        module.fail_json(
            msg="Unable to manage snapshots for non-existing VM %s" % vm_id
        )

    if (
        not module.params["snapshot_name"]
        and module.params["state"] != "remove_all"
    ):
        module.fail_json(
            msg="snapshot_name param is required when state is '%(state)s'"
            % module.params
        )
