...
class VMDKUploader(Thread):
    def __init__(
        self, vmdk, url, validate_certs=True, tarinfo=None, create=False
    ):
    ...
    def get_objects(self):
        ...
        if not self.datacenter:
            self.module.fail_json(
                msg="%(datacenter)s could not be located" % self.params
            )
        ...
        if datastore_cluster_obj:
            ...
            self.datastore = self.find_datastore_by_name(
                self.params["datastore"], self.datacenter
            )

        if not self.datastore:
            self.module.fail_json(
                msg="%(datastore)s could not be located" % self.params
            )
        ...
        if self.params["cluster"]:
            ...
            if cluster is None:
                self.module.fail_json(
                    msg="Unable to find cluster '%(cluster)s'" % self.params
                )
        ...
        for key, value in self.params["networks"].items():
            ...
            if not network:
                self.module.fail_json(
                    msg="%(network)s could not be located" % self.params
                )
    ...
    def upload(self):
        ...
        for i, uploader in enumerate(uploaders):
            ...
            if uploader.e:
                lease.HttpNfcLeaseAbort(
                    vmodl.fault.SystemError(
                        reason="%s" % to_native(uploader.e[1])
                    )
                )
                self.module.fail_json(
...
def main():
    ...
    argument_spec.update(
        {
            "name": {},
            "datastore": {"default": "datastore1"},
            "datacenter": {"default": "ha-datacenter"},
            "cluster": {"default": None},
            "deployment_option": {"default": None},
            "folder": {"default": None},
            "inject_ovf_env": {"default": False, "type": "bool"},
            "resource_pool": {"default": "Resources"},
            "networks": {
                "default": {"VM Network": "VM Network"},
                "type": "dict",
            },
            "ovf": {"type": path_exists, "aliases": ["ova"]},
            "disk_provisioning": {
                "choices": [
                    "flat",
                    "eagerZeroedThick",
            ...
            "wait_for_ip_address": {"type": "bool", "default": False},
            "allow_duplicates": {"type": "bool", "default": True},
