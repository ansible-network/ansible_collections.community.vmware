...
class VMwareDvSwitchPvlans(PyVmomi):
    ...
    def do_pvlan_sanity_checks(self):
        ...
        for primary_vlan in self.primary_pvlans:
            ...
            for primary_vlan_2 in self.primary_pvlans:
                primary_pvlan_id_2 = self.get_primary_pvlan_option(
                    primary_vlan_2
                )
            ...
            if count > 1:
                self.module.fail_json(
                    msg="The primary PVLAN ID '%s' must be unique!"
                    % primary_pvlan_id
                )
        if self.secondary_pvlans:
            ...
            for secondary_pvlan in self.secondary_pvlans:
                ...
                for secondary_pvlan_2 in self.secondary_pvlans:
                    result_2 = self.get_secondary_pvlan_options(
                        secondary_pvlan_2
                    )
                ...
                if count > 1:
                    self.module.fail_json(
                        msg="The secondary PVLAN ID '%s' must be unique!"
                        % result[0]
                    )
            ...
            for secondary_pvlan in self.secondary_pvlans:
                ...
                result = self.get_secondary_pvlan_options(secondary_pvlan)
                for primary_vlan in self.primary_pvlans:
                    primary_pvlan_id = self.get_primary_pvlan_option(
                        primary_vlan
                    )
    ...
    def ensure(self):
            ...
            if self.secondary_pvlans:
                for secondary_pvlan in self.secondary_pvlans:
                    (
                        ...
                        pvlan_type,
                    ) = self.get_secondary_pvlan_options(secondary_pvlan)
                    temp_pvlan = dict()
                    temp_pvlan[
                        "primary_pvlan_id"
                    ] = secondary_vlan_primary_vlan_id
            ...
            if self.dvs.config.pvlanConfig:
                ...
                for primary_vlan in self.primary_pvlans:
                    ...
                    for pvlan_object in self.dvs.config.pvlanConfig:
                        if (
                            pvlan_object.primaryVlanId == primary_pvlan_id
                    ...
                    if self.secondary_pvlans:
                        for secondary_pvlan in self.secondary_pvlans:
                            (
                                ...
                                secondary_vlan_primary_vlan_id,
                                pvlan_type,
                            ) = self.get_secondary_pvlan_options(
                                secondary_pvlan
                            )
                            if (
                                primary_pvlan_id
                                ...
                                for (
                                    pvlan_object_2
                                ) in self.dvs.config.pvlanConfig:
                                    if (
                                        pvlan_object_2.primaryVlanId
                                        == secondary_vlan_primary_vlan_id
                                        and pvlan_object_2.secondaryVlanId
                                        == secondary_pvlan_id
                                        and pvlan_object_2.pvlanType
                                        == pvlan_type
                                    ):
                                        other_found = True
                                        break
                ...
                for pvlan_object in self.dvs.config.pvlanConfig:
                    promiscuous_found = other_found = False
                    if (
                        pvlan_object.primaryVlanId
                        == pvlan_object.secondaryVlanId
                        and pvlan_object.pvlanType == "promiscuous"
                    ):
                        for primary_vlan in self.primary_pvlans:
                            primary_pvlan_id = self.get_primary_pvlan_option(
                        ...
                        for secondary_pvlan in self.secondary_pvlans:
                            ...
                            if (
                                pvlan_object.primaryVlanId
                                == secondary_vlan_primary_vlan_id
                                and pvlan_object.secondaryVlanId
                                == secondary_pvlan_id
        ...
        if changed:
            ...
            if changed_list_add:
                message_add = self.build_change_message(
                    "add", changed_list_add
                )
            if changed_list_remove:
                message_remove = self.build_change_message(
                    "remove", changed_list_remove
                )
    ...
    def get_primary_pvlan_option(self, primary_vlan):
        ...
        if primary_pvlan_id in (0, 4095):
            self.module.fail_json(
                msg="The VLAN IDs of 0 and 4095 are reserved and cannot be used as a primary PVLAN."
            )
    ...
    def get_secondary_pvlan_options(self, secondary_pvlan):
        ...
        if pvlan_type is None:
            self.module.fail_json(
                msg="Please specify pvlan_type in secondary_pvlans options as it's a required parameter"
            )
        elif pvlan_type not in supported_pvlan_types:
            self.module.fail_json(
                msg="The specified PVLAN type '%s' isn't supported!"
                % pvlan_type
            )
        return secondary_pvlan_id, primary_pvlan_id, pvlan_type

    @staticmethod
    def create_pvlan_config_spec(
        operation, primary_pvlan_id, secondary_pvlan_id, pvlan_type
    ):
        """
    ...
    def build_change_message(self, operation, changed_list):
        ...
        if len(changed_list) > 2:
            message = (
                ", ".join(changed_list[:-1]) + ", and " + str(changed_list[-1])
            )
