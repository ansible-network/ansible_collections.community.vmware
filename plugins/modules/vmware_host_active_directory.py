...
class VmwareHostAdAuthentication(PyVmomi):
            ...
            if desired_state == "present":
                ...
                if active_directory_info.enabled:
                    ...
                    if active_directory_info.domainMembershipStatus == "ok":
                        ...
                        results["result"][host.name][
                            "membership_state"
                        ] = active_directory_info.domainMembershipStatus
                        results["result"][host.name][
                            "joined_domain"
                        ] = active_directory_info.joinedDomain
                        results["result"][host.name][
                            "trusted_domains"
                        ] = active_directory_info.trustedDomain
                        results["result"][host.name][
                            "msg"
                    ...
                    # Joined, but problems with the domain membership
                        ...
                        changed = results["result"][host.name][
                            "changed"
                        ] = True
                        results["result"][host.name][
                            "membership_state"
                        ...
                        if (
                            ...
                            msg += (
                                "the host thinks it's part of a domain and "
                ...
                # Enable and join AD domain
                    ...
                    if self.module.check_mode:
                        ...
                        results["result"][host.name][
                            "msg"
                        ] = "Host would be joined to AD domain"
                    else:
                        ...
                        try:
                            ...
                            self.module.fail_json(
                                msg="The domain controller can't be reached : %s"
                ...
                if not active_directory_info.enabled:
                    ...
                    results["result"][host.name][
                        "msg"
                    ] = "Host isn't joined to an AD domain"
                # Disable and leave AD domain
                    ...
                    if self.module.check_mode:
                        ...
                        results["result"][host.name]["msg"] = (
                            "Host would leave the AD domain '%s'"
                            % active_directory_info.joinedDomain
                        )
                    else:
                        ad_authentication = self.get_ad_auth_object(host)
    ...
    def get_ad_info(self, host_object):
        ...
        authentication_store_info = (
            host_object.config.authenticationManagerInfo.authConfig
        )
    ...
    def get_ad_auth_object(self, host_object):
        ...
        authentication_store_info = (
            host_object.configManager.authenticationManager.supportedStore
        )
...
def main():
    ...
    argument_spec.update(
        ad_domain=dict(
            type="str", default="", aliases=["domain", "domain_name"]
        ),
        ad_user=dict(type="str", default=""),
        ad_password=dict(type="str", default="", no_log=True),
        ad_state=dict(
            default="absent", choices=["present", "absent"], aliases=["state"]
        ),
