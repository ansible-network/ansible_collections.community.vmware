    ...
    def ensure(self):
        ...
        if snmp_system:
            if snmp_system.configuration:
                ...
                self.module.fail_json(
                    msg="SNMP agent configuration isn't supported on the ESXi host"
                )
        else:
            self.module.fail_json(
                msg="SNMP system isn't available on the ESXi host"
            )
        ...
        if snmp_state == "reset":
            ...
            results["trap_targets_previous"] = self.get_previous_targets(
                snmp_config_spec.trapTargets
            )
            for option in snmp_config_spec.option:
                if (
                    option.key == "EnvEventSource"
                    and option.value != hw_source
                ):
            ...
            if snmp_config_spec.trapTargets:
                if desired_trap_targets:
                    ...
                    for target in desired_trap_targets:
                        dest_hostname, dest_port, dest_community = self.check_if_options_are_valid(
                            target
                        )
                        ...
                        for trap_target in snmp_config_spec.trapTargets:
                            if trap_target.hostName == dest_hostname:
                                if (
                                    trap_target.port != dest_port
                                    or trap_target.community != dest_community
                                ):
                                    changed = True
                                    changed_list.append(
                                        "trap target '%s'" % dest_hostname
                                    )
                        ...
                        destination = self.build_destination(
                            dest_hostname, dest_port, dest_community
                        )
                    ...
                    for trap_target in snmp_config_spec.trapTargets:
                        ...
                        if not target_found:
                            changed = True
                            changed_list.append(
                                "trap target '%s'" % trap_target.hostName
                            )
                    # Configure trap targets if something has changed
                    if changed:
                        results[
                            "trap_targets_previous"
                        ] = self.get_previous_targets(
                            snmp_config_spec.trapTargets
                        )
                    ...
                    results[
                        "trap_targets_previous"
                    ] = self.get_previous_targets(snmp_config_spec.trapTargets)
                    # Doesn't work. Need to reset config instead
                    # snmp_config_spec.trapTargets = []
                    reset_hint = True
            else:
            ...
            if trap_filter and not event_filter_found:
                ...
                snmp_config_spec.option.append(
                    self.create_option("EventFilter", event_filter)
                )
        ...
        if changed:
            ...
            if not self.module.check_mode:
                try:
                    ...
                    self.module.fail_json(
                        msg="Insufficient resources : %s"
                        % to_native(insufficient_resources)
                    )
        else:
        ...
        if not snmp_state == "reset" and send_trap and desired_trap_targets:
            ...
            if self.module.check_mode:
                ...
                try:
                    ...
                    self.module.fail_json(
                        msg="Error during trap test : Not found : %s"
                        % to_native(not_found)
                    )
                except vim.fault.InsufficientResourcesFault as insufficient_resources:
                    self.module.fail_json(
                        msg="Error during trap test : Insufficient resources : %s"
                        % to_native(insufficient_resources)
                    )
