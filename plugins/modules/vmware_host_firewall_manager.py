...
from ansible_collections.community.general.plugins.module_utils.compat import (
    ipaddress,
)
    ...
    def ensure(self):
        ...
        for host in self.hosts:
            ...
            for rule_option in self.rule_options:
                ...
                current_rule_state = self.firewall_facts[host.name][rule_name][
                    "enabled"
                ]
                ...
                permitted_networking = self.firewall_facts[host.name][
                    rule_name
                ]
                rule_allows_all = permitted_networking["allowed_hosts"][
                    "all_ip"
                ]
                rule_allowed_ips = set(
                ...
                playbook_allowed_networks = set(
                    allowed_hosts.get("ip_network", [])
                )
                ...
                ip_list_different = bool(
                    rule_allowed_ips != playbook_allowed_ips
                )
                ...
                if (
                    allowed_all_ips_different is True
                    or ip_list_different is True
                    or ip_network_different is True
                ):
                    try:
                        ...
                        if not self.module.check_mode:
                            ...
                            firewall_spec.allowedHosts = (
                                vim.host.Ruleset.IpList()
                            )
                            firewall_spec.allowedHosts.allIp = (
                                playbook_allows_all
                            )
                            firewall_spec.allowedHosts.ipAddress = list(
                                playbook_allowed_ips
                            )
                            ...
                            firewall_system.UpdateRuleset(
                                id=rule_name, spec=firewall_spec
                            )
                    except vim.fault.NotFound as not_found:
                        self.module.fail_json(
                        ...
                        self.module.fail_json(
                            msg="Failed to configure the rule set %s as a runtime"
                ...
                results["rule_set_state"][host.name][rule_name] = {
                    ...
                    "allowed_hosts": {
                        ...
                        "previous_allowed_all": permitted_networking[
                            "allowed_hosts"
                        ]["all_ip"],
