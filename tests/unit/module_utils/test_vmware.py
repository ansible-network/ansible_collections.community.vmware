...
testdata = [("HAS_PYVMOMI", "PyVmomi"), ("HAS_REQUESTS", "requests")]
...
@pytest.mark.skipif(
    sys.version_info < (2, 7), reason="requires python2.7 and greater"
)
...
def test_vmdk_disk_path_split(monkeypatch, fake_ansible_module):
    ...
    monkeypatch.setattr(
        vmware_module_utils, "connect_to_api", fake_connect_to_api
    )
